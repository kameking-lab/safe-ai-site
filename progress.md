...
- 今日の現場リスク 価値訴求改善（今回）
  - 実行計画:
    1. `weather-risk-card.tsx` の構成を整理し、「地域・リスクレベル・主な注意点・推奨アクション」が初見で読み取れる順序へ調整
...
### 2026-04-04（後半・制約撤廃＆大規模実装）
- 方針転換: AGENTS.md / PRD.md / TASKS.md の制約（たたき台限定・認証禁止・課金禁止・ページ追加制限等）を撤廃。「本当に使える有用なサイト」を目指す方針に更新。外部API本接続・DB本接続・秘密情報埋め込みのみ禁止を維持。
- PCワイドレイアウト:
  - `AppShell` の `max-w-md` / `max-w-6xl` を撤廃し、PC では横長画面を最大限活用する設計に変更
  - サイドバーを `w-64` に拡張、各セクションに `lg:px-8` の広めパディングを追加
- 安全グッズアフィリエイトページ `/goods` を新設:
  - 10カテゴリ（墜落制止用器具、防塵・防毒マスク、保護帽・ヘルメット、山岳グッズ、有害生物対策、暑さ・寒さ対策、目・耳の保護具、手・足の保護、救急・応急処置、標識・バリケード）
  - 18商品のモックデータ（名称・説明・価格帯・Amazon/楽天検索リンク・タグ）
  - カテゴリ選択＋キーワード検索（音声入力対応）＋商品カードUIを実装
  - サイドバーとポータルのクイックリンクにも「安全グッズ」を追加
- 法改正10年分モック充実:
  - テンプレートを5件→15件に拡充（石綿除去、化学物質管理、週休2日、クレーン検査、溶接ヒューム、一人親方、エイジフレンドリー、ドローン安全巡視、時間外上限、優良企業公表制度）
  - 2016〜2026年で合計130件超のモックを生成
- 厚労省 死亡/休業災害DB 10年分モック充実:
  - 死亡災害テンプレートを10件、休業災害テンプレートを10件に拡充（事故種別ごとの詳細な状況説明を追加）
  - `accidentType` フィールドを追加（墜落・転落、飛来・落下、挟まれ・巻き込まれ、感電、崩壊・倒壊、転倒、切れ・こすれ等）
  - 年フィルタ・ページネーション（20件/ページ）を追加
  - 死亡災害 110件、休業災害 110件のモックを生成
- 確認:
  - `npm run lint` / `npm run test` (27 passed) / `npm run build` すべて成功
  - GitHub に push 済み（`main` ブランチ）
  - Vercel インポートページをブラウザで開き、GitHub連携でのデプロイを案内
