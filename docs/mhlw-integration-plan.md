...
### ステップ 2e: 仕上げ（縦書きPDF再抽出 / 化学物質RA連携 / 業種ランキング / 類似事例） ✅ (2026-04-19 完了)

2d で UI 上は出揃ったが、残課題（縦書き PDF の articleNumber 欠損、
`/chemical-ra` への濃度基準値連携、`/risk-prediction` での MHLW 実データ活用、
`/accidents` の業種別ドリルダウン）を 4 コミットに分けて仕上げた。

#### 1. 法令 PDF 縦書き抽出の改善

- `scripts/etl/parse-laws-pdf.py` を pypdf から **pdfplumber** に切替。
  既定 `extract_text` 結果に「CJK 文字直後が空白」のパターンが 50% 以上
  含まれる場合は縦書きと判定し、char 単位で右→左の列順に再構成する
  `_extract_vertical()` を追加。
- 条文検出 regex に `附則` / `別表` / `経過措置` を追加。条見出しを
  跨いだ前文ページは前ページの条文に紐付ける（`last_article` 継承）。
- 結果: 11 PDF 全量パースで articleNumber タグ付きが **339 → 1,414 件**
  （build-laws-compact 後の有効条文は **295 → 1,127 件**）。
- commit: `feat(laws): improve vertical PDF extraction with pdfplumber`

#### 2. /chemical-ra に濃度基準値連携

- `scripts/etl/build-chemicals-compact.mjs` で `八時間濃度基準値`/`短時間濃度基準値`/
  `モデルSDSにおける推奨用途`/`リンクURL` を `details` フィールドとして保存。
  195 物質に数値付き濃度基準値が紐づく。
- `web/src/lib/mhlw-chemicals.ts` に `searchMergedChemicals()`、`findByCas()`、
  `regulatoryLabels()`、`relatedLawTexts()` を追加。
- `web/src/components/mhlw-chemical-selector.tsx` (新設) — 1,389 物質から
  名称・CAS で検索するドロップダウン。物質名直接入力フィールドと併設。
- `web/src/components/mhlw-chemical-info-card.tsx` (新設) — 選択時に
  8h / 短時間 濃度基準値、SDS義務 / 皮膚 / がん原性 / 濃度フラグ、
  規制区分（自律的管理 / SDS交付義務など）、関連法令、用途、SDS PDF リンクを表示。
  測定濃度の入力欄で 8h 基準値超過判定（倍率付き）も実装。
- `/chemical-ra?cas=NNN-NN-N&name=…` URL パラメータで起動時自動選択。
- commit: `feat(chemical-ra): link concentration limits from MHLW data`

#### 3. /chemical-database と /chemical-ra のクロスリンク

- `mhlw-chemical-aggregated-panel.tsx` 各物質詳細に
  「この物質のリスクアセスメントを実施」ボタン → `/chemical-ra?cas=...`。
- `chemical-database-client.tsx` 専門解説 50 物質にも同ボタンを追加。
- commit: `feat: add cross-links between /chemical-database and /chemical-ra`

#### 4. 業種別危険度ランキング + MHLW 類似事例

- `scripts/etl/build-industry-profiles.py` (新設) — 504,415 件の raw JSONL から
  業種別の `total` / `topTypes`(上位5型) / `ageProfile`(年齢分布) を集計し、
  `aggregates-mhlw/industry-{ranking,profiles}.json` (合計 ~20 KB) を生成。
- `web/src/components/industry-risk-ranking.tsx` (新設) — `/accidents` に
  「業種別ランキング」タブを追加。TOP 20 横棒チャート（クリックで選択）、
  「あなたの業種の統計を見る」チップ、選択業種の事故型 TOP5 と年齢分布。
- `web/src/lib/mhlw-similar-cases.ts` (新設) — 4,043 件の死亡災害 compact
  に対して、CJK / カタカナ / 英数字トークン分割と本文 / 事故型 / 業種 / 原因の
  重み付け（5/4/3/2/1）で簡易類似度検索する `searchMhlwSimilar()`。
- `web/src/components/mhlw-similar-cases-panel.tsx` (新設) — `/risk-prediction`
  検索後に MHLW 実データ TOP 5 を表示。一致トークンと類似度バー付き。
  既存 300 件のサイトモックは別セクションとして併存。
- commit: `feat: industry risk ranking and improved risk prediction with MHLW data`

#### 仕上げ後に解消した残タスク

- 「法令 PDF の再抽出」（2c の残タスク #5） → 完了
- 「/chemical-ra に濃度基準値連携」（フェーズ 5 の残タスク #3）→ 完了
- /accidents の業種別ドリルダウン → 新規実装で達成
- /risk-prediction を MHLW 実データに広げる → 新規実装で達成

#### 仕上げ後も残る項目

- フェーズ 2b: Blob トークン設定 + 50 万件シャード投入（オーナー作業）
- フェーズ 3: 法令 RAG / 埋め込み（API キー要オーナー確認）
- 個別詳細ページ `/accidents/[id]`（Blob 投入後）
- 化学物質関連 PDF (`shishin*.pdf` 等) の RAG 取り込み
- LAW_NAME_HINTS の正式名称への差し替え（PDF 中身の確認が必要）
...
3. ~~`/chemical-ra`（リスクアセスメント）で選んだ物質の濃度基準値を自動反映~~
   → **2e で完了**。1,389 物質ドロップダウン + 8h/短時間 濃度基準値カード +
   測定濃度の超過判定を実装。
...
- ~~commit 2: `feat(chemical-ra): auto-populate exposure limits from MHLW concentration standards`~~ → 2e で完了
...
Phase 0 (done) ✅
    ...
    │       ├── Phase 2e (縦書きPDF再抽出・/chemical-ra連携・業種ランキング・類似事例) ✅
    │       │       └── 残: /accidents/[id], モック整理, PDF URL マップ
