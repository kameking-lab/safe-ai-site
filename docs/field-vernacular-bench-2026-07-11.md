...
## 6. 本番51問eval実測（PR #875 マージ・Vercel反映後 2026-07-11T15:50Z）

`CHATBOT_EVAL_BASE_URL=https://www.anzen-ai-portal.jp CHATBOT_EVAL_INTERVAL_MS=18000
npm run eval:chatbot-gen`（全51問HTTP 200・429なし）。

- **完全正答: 46/47 = 97.9%**（ベースライン 2026-07-11T11:49Z の 45/47=95.7% から向上）
- **GQ48（クビ→解雇予告）: ×→○**（労基法20条到達・「30日」結論・偽警告なし）
- **GQ49（マンホール→酸欠資格）: △→○**（酸欠則11条到達・gold引用あり）
- プレースホルダ漏出0・偽の範囲外警告0
- 残る×は2件:
  - **GQ20（派遣の雇入れ時教育）**: 誤結論regex検出。PR #873 の本番実測でも同一の既存事象
    （生成層のゆらぎ）と記録済み＝本件retrieval是正と無関係・非退行。
  - **GQ51（車検）**: ドメイン外語減点は効いた（confidenceScore 0.62→0.31・no-hitテンプレ化・
    confidence low）が、no-hit応答の「関連する可能性のある条文」ブロックが偶発ヒット
    （騒音規制法16条等）をcitations付きで残し、採点上「条文引用つき回答」のまま。

### GQ51フォローアップ是正（同日）

両APIルート（`route.ts`・`stream/route.ts`）のno-hit関連条文の提示に
