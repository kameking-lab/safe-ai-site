...
- 実名「[redacted]」「[redacted]」: 私の追加分（YAML / 詳細ページ）に**ゼロ**（既存 docs/handover に残存だが本タスク範囲外）
- 監修者文言: 全 11 新規 YAML / 全 11 新規詳細ページで「安全AIポータル ／ 労働安全コンサルタント（登録番号[redacted]・土木）監修」固定
