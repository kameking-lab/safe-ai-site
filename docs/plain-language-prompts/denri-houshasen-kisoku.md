...
【役割】安全AIサイトの「現場ことば版」執筆担当。担当は **電離放射線障害防止規則（電離則）** の1法令のみ。
コーパス収載の全条（約20条）を、現場の人でも理解できる端的な言い換えにして、
...
3. 原文: web/src/data/laws/denri-houshasen-kisoku.ts（＋ web/src/data/laws/corpus-gaps-fill.ts に
   電離則の条があればそれも対象）
4. 用語集の見出し語: web/src/data/glossary/（専門語はこの表記に合わせる）

【手順】
1. `git checkout main && git pull --ff-only` → 作業ブランチ `plain/denri-houshasen-kisoku` を作成。
2. ハッシュ取得: `cd web && node scripts/plain-source-digest.mjs src/data/laws/denri-houshasen-kisoku.ts`
   （corpus-gaps-fill.ts に担当条がある場合はそちらにも実行）
3. `web/src/data/plain/denri-houshasen-kisoku.ts` を新規作成し、
   `export const plainDenriHoushasenKisoku: PlainArticle[]` に全条を執筆。
...
5. fidelityテストを自分で回して全緑にする:
   ...
   反映して再実行。**原文を変えるのは禁止**。
6. `cd web && npm run plain:status` で 電離則 の行が
   「済(fresh)=電離則の収載条数・未生成0」になることを確認。
...
- 捏造0（原文に無い義務・数値・条件を書かない。fidelityゲートが検出するが、
  ゲートを騙す書き方も禁止）
- 触るのは web/src/data/plain/denri-houshasen-kisoku.ts と index.ts の登録1行のみ
