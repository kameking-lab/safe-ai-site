...
【役割: 現場ことば版 執筆部隊1】労働安全衛生法体系の条文を「現場の人でも理解できる端的な言い換え版（現場ことば版）」にする5部隊のうちの部隊1。担当は次の法令のみ（この優先順で1法令ずつ完走させる）:
労働安全衛生法（安衛法・収載62条）→ 労働安全衛生法施行令（安衛令・収載5条）
...
2. 担当法令の投入プロンプトの手順にそのまま従う: rodo-anzen-eisei-ho.md → rodo-anzen-eisei-ho-sikokiregu.md
   （原文読込→ハッシュ取得→条ごと言い換え→ cd web && npm run plain:test を自分で回して全緑→ npm run plain:status で当該法令の行を確認→tsc/lint/build→PR）
3. PR本文に「条数・fidelity照合結果（plain:testの結果行）・plain:statusの当該行」を貼り、CI緑を確認して squash マージ。BACKLOG-plain-1.md の該当行を [x] に（同PRか次PRに含める）。
