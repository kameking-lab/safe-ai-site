...
## 2026-09-25 再接続と特徴選択の進捗

- 本番Vercelの環境変数名だけを再確認した。`NEXT_PUBLIC_RAKUTEN_AFFILIATE_ID` はあるが、商品検索APIに必須の `RAKUTEN_APPLICATION_ID` と `RAKUTEN_ACCESS_KEY` はない。値の閲覧・記録はしていない。現行[楽天API仕様](https://webservice.rakuten.co.jp/documentation/ichiba-item-search)にも両方必須と明記されているため、本番実写真・実測レビュー表示は接続待ちのまま。表示できない状態を成功に数えない。
- 保護帽、フルハーネス、呼吸用保護具、保護眼鏡のカテゴリを開いた後に、先に用途・危険の特徴を選ぶ。検索語はサーバー側の固定候補から選び、カテゴリに異なる品目をANDで混ぜない。物質・酸素濃度が不明な呼吸用保護具は販売候補を出さない。検索結果は規格適合の保証ではなく、各選択に購入前の確認点を示す。
- URLの`category`と`feature`で選択状態を保持し、戻る操作・再読込に対応する。横比較の位置は同じタブのsessionStorageで保持し、保存不可なら通常操作を続ける。失敗時は認証・回数制限・タイムアウトを区別し、欠測を高評価に変換しない。成功応答のみサーバー内で最大5分キャッシュし、取得日時は実取得時点を表示する。
- 写真は楽天APIが返すURLを直接表示する実装を維持し、静的な架空の商品写真・星評価・スクレイピング画像は追加していない。[楽天のクレジット規則](https://webservice.rakuten.co.jp/guide/credit)に従った表示とアフィリエイトリンクを保持する。128px角のAPI画像のため、拡大写真や全規格の照合は販売先・メーカー資料へ進む必要がある。

**再開条件:** 運営者の楽天ウェブサービスアプリで当サイトの利用が有効になり、上記2変数をVercel Productionへ設定した後、再デプロイする。担当者が保護帽・フルハーネス・呼吸用保護具・保護眼鏡の4カテゴリで、取得日時・商品名・写真・評価/件数・販売URLを実物と照合する。星4.2以上・レビュー10件以上でも型式の規格適合は別確認する。
