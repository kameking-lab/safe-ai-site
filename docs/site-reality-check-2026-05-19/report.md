...
2. F-002(ハードコード認証鍵)は 2026-05-19 時点で本番で実際に動作しており、curl で /api/admin/health?key=<旧固定鍵> を叩くと内部の Gemini/Stripe/Supabase/GA4/GSC 等10サービスの configured 状態・fallback 動作・circuit breaker 状態 を含む詳細 JSON を返却していた。security/f002-admin-health-auth で `process.env.ADMIN_HEALTH_KEY` に環境変数化。Vercel 本番に新しい強い乱数値を設定する作業が残る（未設定時は 401）。
