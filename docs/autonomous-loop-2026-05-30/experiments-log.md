...
---
## R6 労災裁判例コーナー さらに拡充（じん肺国賠・役員個人責任）— 2026-05-31 早朝

**実在裏取りできた2件のみ追加（14→16件）**。

追加:
1. **筑豊じん肺訴訟**（最高裁三小 平16.4.27・裁判例検索 id=52326）— 炭鉱じん肺につき国（通産大臣）の鉱山保安法に基づく規制権限不行使を国家賠償法上違法と判断。長期発症型の損害は民法724条後段の期間制限を損害発生時から起算。裏取り: 裁判所裁判例検索・経産省ページ・CiNii解説。
2. **大庄（日本海庄や）事件**（大阪高裁 平23.5.25→最高裁 平25 上告退け確定）— 過労死につき会社の安全配慮義務違反に加え、長時間労働を前提とする勤務体制を維持した取締役個人の責任（会社法429条1項）を認めた稀少判断。新カテゴリ「役員・個人責任」。裏取り: 労働新聞社判例解説・日経（最高裁確定報道）。

検証: lint 0 / tsc 0 / 判例整合テスト 9→10（16件以上・役員個人責任カテゴリ・追加2件の出典/事実記述）/ build OK。一覧16件・新カテゴリ・追加2件詳細・A4まとめ印刷(16件)を Playwright で確認。

R5+R6 計4件追加（12→16）。スタンスは一貫: 件数より真正性。確証取れた本物だけを足し、特定できない論点（はさまれ/崩壊/感電/建設墜落の追加最高裁）は正直に見送り。
