...
---
## R5 労災裁判例コーナー拡充（建設墜落・熱中症・派遣先責任）— 2026-05-31 早朝

社長指示で論点拡充。**実在裏取りできた2件のみ追加（12→14件）**。確証なきものは載せない原則を継続。

追加（全件 複数の信頼出典で裏取り）:
1. **アテスト（ニコン熊谷製作所）事件** — 東京高判 平21.7.28、最高裁 平23.9.30 上告等退け**確定**。偽装請負（違法派遣）で受け入れた労働者の過労自殺につき、受入先（派遣先・発注者）と雇用元 双方の安全配慮義務違反を認定。新カテゴリ「派遣・請負先責任」。裏取り: 働くもののいのち全国センター(inoken)事件経過・労働新聞社判例解説・各報道で 地裁(平17.3.31)→高裁(平21.7.28)→最高裁確定(平23.9.30) を確認。
2. **造園作業中の熱中症死亡事件（大阪高裁 平28.1.21）** — 勤務初日に猛暑下の伐採・清掃で熱中症発症・死亡。症状認識後の休憩・冷却・救急要請の遅れ、監督者への熱中症対応教育の欠如に安全配慮義務違反。新分野「熱中症・屋外」。裏取り: 弁護士池田剛志解説(大阪高裁平28.1.21を名指し)ほか複数法律事務所の解説でWBGT33・造園・約3600万円・holdingを確認。

スキップ（確証取れず・捏造防止）:
- 建設足場墜落の追加最高裁判例（最判平3.10.25 と思しきもの）: 事件名・holdingを一次情報で特定できず不採用。建設墜落は既存「大石塗装・鹿島建設事件」で担保。
- はさまれ・崩壊・感電の確定判例: 明確に特定できる代表判例が見つからず不採用。
- 福岡地裁小倉 R6.2.13 熱中症（新星興業）: 地裁・ごく最近で確定未確認のため、より上級・定着した大阪高裁H28を採用し見送り。

検証: lint 0 err / tsc 0 / 判例データ整合テスト 8→9（14件以上・新カテゴリ・追加2件の出典/事実記述を検証）/ 全989→990 pass / build OK。
