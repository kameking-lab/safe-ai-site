...
## 2026-07-03 柱C-3-3補充（自領域）RSS登録簿 feeds.ts の逆方向 drift ガード（#796）

**背景**: 未着手[ ]タスクなし（O17/T6 は Path A 設計ドラフト化済＝実装オーナー承認待ち）＋自分の open PR #791(⌘K e-Gov 条アンカー)は CI 進行中で未マージ。3件未満のため補充の指針§に従い自領域から補充。#791 とはファイル非重複（feeds vs CommandPalette）＝padding 回避。

**変更**: `lib/seo/feeds.test.ts` は既に SITE_FEEDS 登録簿→実ルート(route.ts)の「幽霊フィードリンク0」を機械突合していたが、逆方向（`app/feed/<slug>.xml/route.ts` を新設したのに SITE_FEEDS へ登録し忘れる欠落）は素通しだった。未登録フィードはクロール可（robots で /feed 未Disallow）でもどのページ head の rel="alternate"(application/rss+xml) にも載らず**自動発見不能**＝feeds.ts の docstring が是正対象と明記した穴そのもの。`app/feed` 配下の全 *.xml ルート（GET を持つ route.ts）を readdirSync 走査し全件 SITE_FEEDS 登録済みを機械固定する逆方向ガード 1 it を追加（走査空振り防止のサニティ≥2本）。現状4本(news/law-revisions/accident-reports/serious-cases)全登録済み＝緑。**コード変更0**＝既存挙動の特徴づけ固定のみ（manifest 資産実在ガード・sitemap ゴーストURLガードと同方針・水増し/捏造0・既存破壊0）。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run`=全2581テスト緑（feeds.test.ts 6 it・新規1）/ `build`=成功。

**残**: #791（⌘K e-Gov 条アンカーパリティ）は CI 緑を次イテレーション 1) で回収＆マージ。#796（本PR）も同様。O17/T6 実装はオーナー承認待ち。
