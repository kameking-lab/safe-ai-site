...
## 2026-07-03 sitemap/robots のドメインを SITE_URL 単一ソース化（柱C-3 / S DRY）

前イテレーションでマージした #698（page-json-ld のドメイン集約）に続き、発見性インフラの中核 sitemap/robots に残っていた最後のドメイン直書きを是正。og-image(#710)・json-ld(#530)・page-json-ld(#698) は既に `seo-metadata.ts::SITE_URL` へ集約済みだったが、`web/src/app/sitemap.ts`（`const base`）・子sitemap 6本の route.ts（`const BASE`＝sitemap-index/articles/accidents/chemicals/circulars/equipment.xml）・`robots.ts` の Sitemap 行、計8箇所が `https://www.anzen-ai-portal.jp` を直書きしたまま取り残されていた。これは `SITE_URL` 変更時に sitemap 全 `<loc>` と robots の Sitemap 参照が旧ドメインへ無言ドリフトし、新ドメインで sitemap が全滅＝クロールが全停止する最も痛いクラスのドリフト穴。

対応: 8ファイル全てで `import { SITE_URL } from '@/lib/seo-metadata'` し、エイリアス（base/BASE）の値のみ `SITE_URL` へ差し替え（名前は温存）。`SITE_URL` は末尾スラッシュ無し＝従来の直書き値と完全同値のため出力は byte-identical（各 route の既存 route.test.ts が無回帰を担保）。

回帰: `sitemap-domain-single-source.test.ts` を新設（10 it）＝(1) it.each で対象8ファイルにドメイン直書きリテラルが無く `@/lib/seo-metadata` から `SITE_URL` を import するハードコードガード、(2) `sitemap()` の全 `<loc>` が SITE_URL オリジン始まり、(3) `robots()` の Sitemap 行が `${SITE_URL}/sitemap-index.xml`。

ゲート: `tsc --noEmit`=0 / `lint`=errors0（既存 warning のみ・当該ファイル0）/ `vitest run`=284ファイル2423テスト全pass（新規10 it 含む）/ `build`=成功。build 再生成物（rag-metrics-latest.json・chatbot-eval-fresh-results.json）は `git checkout` で復元。working tree は当領域8ファイル＋新規テスト1本のみ。

残: 本 PR #711 の CI 緑回収＆マージ（次イテレーション 1)）。#710 も CI 待ち。#704 は CONFLICTING（要 origin/main マージ解決）。未着手キューは空＝次も補充（自領域 SEO/構造化データ）。
...
## 2026-07-03 柱C-2 ⌘K 横断検索の0件フォールバックを /search とパリティ化（安全ガードの第2入口展開）

**背景/穴**: 横断検索は /search（結果ページ）と ⌘K（CommandPalette）の2入口が同一インデックス（search-index.ts）を共有するのに、0件時UXが乖離していた。/search は T4（診断書 05-search-egov.md G7）で「収録範囲の明示（未収載≠規定なし）＋e-Gov 一次情報への逃がし＋trackEvent」を備えるが、⌘K は素の『結果が見つかりませんでした』のみ＝curated 抄録に無い条番号を打った現場ユーザーへ「その規定は存在しない」と誤読させる安全リスクが第2入口に残っていた。

**是正**: `CommandPalette.tsx` の0件ブロックへ /search の NoResults と同一方針の安全ガードを追加＝amber の収録範囲注記（見つからない＝規定がないではない）＋e-Gov 法令検索リンク（`@/lib/cross-search` の `EGOV_LAW_SEARCH_URL` を再利用＝到達可能なポータルトップ固定で幽霊リンク0）＋`trackEvent('search_zero_result_egov')`（/search と同一イベント名で両入口の0件計測を統一）。lib ヘルパー再利用で実装重複なし。検索/メタ以外の可視改変なし。

**回帰**: `CommandPalette.test.tsx` 新設（2 it・RTL で実コンポーネントを 0-hit クエリまで駆動）＝e-Gov リンクの href/target/rel と安全コピー描画を固定。jsdom 未実装の scrollIntoView を no-op stub。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run`=290ファイル2469テスト全pass（新規2含む）/ `build`=成功。working tree は CommandPalette.tsx / CommandPalette.test.tsx / BACKLOG-seo.md / 本ログの範囲。

**残**: 本 PR の CI 緑回収＆マージ（次イテレーション 1)）。#729(JSON-LD @id follow-up)・#738(法改正記事の横断検索収載) の緑回収も継続。T5(条文参照リンク)は条文本文表示＝UI班領域・T6/T7 は外部API取込みでオーナー承認待ち。

---

## 2026-07-03 柱C-2 横断検索に法改正記事(10本)を収載（#738）＝発見性の穴を是正

**背景**: site-critique `01-seo-technical` S-1「法改正記事という検索需要のあるコンテンツ枠が丸ごと死んでいる」。法改正記事（`src/data/articles/*.json`＝監修済みの法改正・実装ガイド解説 10本）は `/articles` 一覧と `sitemap-articles.xml` には載っているのに、横断検索（/search・⌘K）から**丸ごと0件ヒット**だった。原因は正本 `getPublishedArticleIndex` が `node:fs`(readdirSync) 依存で**ブラウザ非安全**＝client 検索インデックス(search-index.ts→CommandPalette は 'use client')から import 不可で、過去 iteration で「要・他班（data班のブラウザ安全集約待ち）」と保留していた穴。今回はバンドル肥大・drift の懸念を **動的import(遅延チャンク)＋drift ガード** で解いて自班で結線した。

**変更**: (1) **`lib/articles-search-source.ts` 新設**＝10本の記事 JSON を静的import(fs非依存＝ブラウザ安全)し、本文(sections/sources)を除いた軽量エントリ `ARTICLE_SEARCH_ENTRIES` へ射影。search-index からは**動的import**されるため遅延検索チャンク側にのみ載り、メインバンドルは非肥大(court-cases 全件・compact.json 919物質を既にロードする同じチャンク)。(2) `search-index.ts` に `article` カテゴリ新設(CATEGORY_META=violet・SEARCH_CATEGORIES・SEARCH_CATEGORY_PRIORITY=FAQ の次点＝条文/教育/FAQ には譲る・countByCategory)。url は `/articles/<slug>` 深リンク＝`/articles/[slug]` の generateStaticParams(`getPublishedArticleSlugs`)が公開済み全 slug を解決するため**必ず着地(幽霊URL 0)**。時限公開(publishedAt が未来)は実行時 now で除外＝正本と同セマンティクス。(3) UI(SearchResults/CommandPalette)に 記事 タブ＋Newspaper アイコン追加。(4) `/search` メタ description を実カバレッジ(法令条文・法改正記事…)へ更新＝旧5カテゴリの陳腐化是正。

**drift ガード**: 静的import は列挙を自動追従しないため、`articles-search-source.test.ts`(3 it)で**実在ファイル集合(readdirSync)と slug 集合の双方向一致**を機械固定＝data班の記事追加/改名/削除を「横断検索へ結線し忘れる穴」を CI で発見性側へ強制(sitemap ゴーストURL回帰ガードと同方針)。`search-index.test.ts` に article describe(3 it)＝公開済み全件収載・深リンク slug が `getPublishedArticleSlugs` へ双方向一致(soft404 0)・タグ/キーワードヒット(熱中症/フルハーネス)。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0(既存の無関係warn1のみ) / `vitest run`=2467 pass・1 skip(新規6含む) / `build`=成功。build 再生成物(rag-metrics-latest.json・chatbot-eval-fresh-results.json)は `git checkout` で復元。working tree は自班7ファイルのみ。

**残**: #738 の CI 緑回収＆マージ（次イテレーション 1)）。個別記事の JSON-LD(NewsArticle)・本文SSRは /articles ページ所有UI班の担当。O17/T6・T7 実装はオーナー承認待ち。

---

## 2026-07-03 柱C-4・S JSON-LD 低頻度 provider/creator ノードの @id 集約（#704 follow-up 消化）

**背景**: #704 で主要エンティティ（Organization/WebSite/Article/NewsArticle/WebApplication/WebPage）は正準 `ORG_ID`/`WEBSITE_ID` へ @id 集約済みだが、同 PR が明記した follow-up 候補どおり、低頻度スキーマの `provider`/`creator`/`isPartOf` が name/url をインライン再宣言したまま取り残されており、検索エンジンから「同一の自サイト組織/サイト」が別ノードへ分裂＝ナレッジグラフのブランド同定シグナルを希釈する #704 と同型の残穴だった。BACKLOG-seo.md の未着手キューは実装タスク0（O17/T6/T7 は Path A 承認待ち）のため、補充の指針に従い自領域から本 follow-up を補充。

**変更**: `json-ld.tsx` に軽量参照 `ORG_REF`（`@type:Organization`＋`@id:ORG_ID`＋name/url・ロゴ無し）/`WEBSITE_REF`（`@id:WEBSITE_ID`）を新設。`serviceSchema.provider`・`courseListSchema` の Course.provider・`datasetSchema.creator`・`quizSchema.provider`・`dataCatalogSchema.creator` を全て `ORG_REF` 参照へ集約。`qaPageSchema.isPartOf` を `WEBSITE_REF`＋新規 `publisher:ORG_REF` へ（@id 未同定＋publisher 欠落を是正）。`webPageSchema` の既存インライン @id ブロック（isPartOf/publisher）も同 REF へ DRY＝出力 byte 不変。全ノードが `PUBLISHER_REF`（ロゴ付き発行主体）と同一 @id を共有し 1 つの正準 Organization/WebSite ノードへ収斂。

**不変条件**: JSON-LD の意味論的リンク付け（@id 参照）のみの追加＝可視UI/ルーティング/メタ本文は無改変。捏造0・既存破壊0・投機的拡張なし（provider/creator の新規追加はせず既存ノードの @id 同定のみ。qaPage の publisher 追加は webPage の正準パターンへの整合）。

**回帰**: `json-ld.test.ts` に describe「エンティティグラフ: 低頻度 provider/creator ノードの @id 集約」4本追加＝Service.provider・Dataset/DataCatalog.creator・Quiz.provider・QAPage.isPartOf/publisher が全て正準 `ORG_ID`/`WEBSITE_ID` を指すことを `toEqual` で固定。既存 CourseList provider テストを @id 付きへ更新（strict `toEqual`）。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run`=286ファイル2446テスト全pass（json-ld.test は 35 pass）/ `build`=成功。working tree は json-ld.tsx / json-ld.test.ts の2ファイル＋BACKLOG/cycle-log。

**残**: 本 PR の CI 緑回収＆マージ（次イテレーション 1)）。前イテレーション PR #724（sitemap ゴーストURL回帰ガード）も CI 待ち回収。O17/T6・T7 実装はオーナー承認待ち。記事横断検索は要・他班（data班ブラウザ安全集約待ち）。

---

## 2026-07-03 柱C-4 JSON-LD follow-up の重複PR統合＋WebSite.author @id 集約（#729 に一本化・#719 supersede）

**契約1) 回収**: 前イテレーションの緑PRを回収＝#713（sitemap/robots ドメイン単一ソース化）を squash マージし main を ff-only 更新。#724（sitemap ゴーストURL回帰ガード）は BACKLOG 追記衝突を origin/main 通常マージで解決し再push（CI 再実行→次イテレーション回収）。

**重複検知＆是正（水増し回避）**: 自分の未マージPR #719 と #729 が **#704 の同一 follow-up（低頻度 provider/creator/isPartOf ノードの @id 集約）の別実装で ~95% 重複**していることを json-ld.tsx の branch 間 diff で検出。二重に「完了」計上するのは水増しに当たるため、**単一の正準実装 #729 へ統合**する方針を決定。各PR固有の差分＝(a) #719 のみ: `webSiteSchema.author`（全ページ出力・別 url `/about` のインライン Organization＝別ノード分裂）の @id 集約、(b) #729 のみ: `qaPageSchema.publisher` 追加＋軽量 `ORG_REF` ヘルパー。(a) を #729 へ取り込み（`author` を sibling publisher と同形の bare `{ "@id": ORG_ID }` へ）、`json-ld.test.ts` の webSiteSchema @id テストに `author=ORG_ID` 断言を追加。#719 は理由付きでクローズ＋リモートブランチ削除。

**実測（end-to-end）**: `npm run build` 後の prerendered `index.html` の WebSite スキーマが `"author":{"@id":"…/#organization"}` を出力＝正準 Organization ノードへ収斂することを grep で確認（可視UI/ルーティング/メタは無改変）。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run`=287ファイル2458テスト全pass（json-ld.test は 35 pass）/ `build`=成功。

**残**: #729・#724 の CI 緑回収＆マージ（次イテレーション 1)）。O17/T6・T7 実装はオーナー承認待ち。記事横断検索は要・他班（data班のブラウザ安全集約待ち）。

---

## 2026-07-03 柱C-2 横断検索の化学物質ヒットを canonical 個別詳細へ深リンク（PR #786）

**背景**: BACKLOG-seo.md 未着手0件のため補充（自領域＝横断検索）。survey で発見した実在の穴＝横断検索(/search・⌘K)の化学物質ヒットが mock50・MHLW約3,954件とも一覧クエリページ `/chemical-database?q=<名称>` へ落ち、`sitemap-chemicals.xml` 収載の3,515本の canonical 個別詳細（サイト最大級の独自コンテンツ）を素通りしていた。事故/保護具/通達は既に個別詳細へ深リンク済みなのに化学物質DBだけ取り残された内部リンク＋UXの二重欠落。直近2PR（#780/964254e8）がシノニム系だったため、near-dup 回避のため別種の distinct タスクを選択（[[avoid-overlapping-followup-prs]]）。

**実装**: 正本＝濃度基準DB `concentration-limits.json` のキー集合（詳細ページが `notFound()` で弾く境界＝実在CASの全体）。本体約2.0MBは client 検索チャンクへ載せられないため**キーのみ40KB/gz13KB**を軽量スナップショット `src/lib/cross-search/chemical-detail-cas.json` として抽出（generator `scripts/gen-chemical-detail-cas.mjs`、出力は data班所有の src/data を避け当班lib配下）。純関数 `chemicalDetailUrl(cas,name)`＝収載CASは `/chemical-database/<cas>`（sitemap URL と1:1一致）、未収載約4%は一覧クエリへフォールバック＝幽霊URL 0。実測overlap: compact 2481/2595・mock 48/50 ≒95%。

**ドリフト検出**: `chemical-detail-cas.test.ts` がスナップショット⇔濃度基準DBキー集合の完全一致をCIで固定＝data班のDB更新で乖離したら赤化して再生成強制（silent soft404 封止）。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run`=全2562テスト pass（新規13）/ `build`=成功。再生成データ（docs/rag-metrics-latest.json・chatbot-eval-fresh-results.json）は復元。

**残**: PR #786・#780 の CI 緑回収＆マージ（次イテレーション 1)）。

---

## 2026-07-03（イテレーション・SEO班）横断通達検索の現場語彙橋渡し

**回収**: #767（e-Gov 条アンカー0件フォールバック）を CI 緑確認のうえ squashマージ→main を ff-only 同期。#772（manifest 資産実在ガード）は追記衝突を origin/main 通常マージで解決し再push（BACKLOG-seo 完了欄の追記競合のみ・両側保持）。#774（feed 逆カバレッジガード）は CI 進行中で次イテレーション回収。

**本題**: 補充の指針（未着手0件）に従い自領域 C-2 から補充。docs/site-critique-2026-06-11 の SEO テクニカル項目（S-1 記事シェル/A-2 内部レポート露出/A-4 旧equipment ID/C-1 og:image フォールバック）は本番・コードとも既に是正済みを確認（水増し回避で再着手せず）。代わりに `notice-search.ts`（安衛法チャットボットの関連通達添付・当班 C-2 所有）の `TOPIC_SYNONYMS` に**現場口語・旧法令名・障害名の橋渡し**を追加＝「酸欠」（通達タイトルに literal 0 件・正式表記は酸素欠乏）・「振動障害」（0 件・原因作業はチェーンソー）・旧称「安全帯」（2019 改称→墜落制止用器具）が現場語彙のままでは通達 0 件になる取り逃しを、実データ裏付けを取って 3 群新設＋教育群に職長追記で是正（石綿↔アスベストと同型）。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run`=299ファイル2557テスト全pass（notice-search.test は新規5含む14 pass）/ `build`=成功。捏造0（各語はタイトル実在に裏付け）・既存破壊0（加点方向のみ・既存テストのトリガ語に無影響）。

**残**: #774 の CI 緑回収＆マージ（次イテレーション 1)）。#772 は再push後 CI 再走行中。O17/T6 実装はオーナー承認待ち。

---

## 2026-07-03 柱C-3-3 補充 PWA manifest メディア資産（icons/screenshots）実在ガード新設

**背景**: `manifest.test.ts` はショートカット URL の実在（幽霊ショートカット防止）は突合していたが、`icons[].src`・`screenshots[].src` の public/ 実ファイル参照は無ガード。他班が public/ 画像をリネーム/削除しても参照文字列は残り、リンク切れは PWA インストール時にしか露見しない。とりわけ 192px+512px の png icon 欠落は Chrome のインストール可能性（beforeinstallprompt）を丸ごと殺すのに CI 無検知＝発見性（ホーム画面設置導線）の静かな死。

**変更**: ショートカット実在ガードと同型の `publicAssetExists(src)`（`public/` 配下 existsSync 突合）で describe「メディア資産 実在ガード」4 it を新設＝(1)全 icon src 実在＝幽霊アイコン0 (2)192/512 png icon 実在（インストール必須要件）(3)全 screenshot src 実在＝幽霊スクショ0 (4)wide/narrow 両 form_factor 具備。**コード変更0**＝現状全資産は実在（緑）で、将来の資産削除/改名を赤化させる回帰ガードのみ（水増し・捏造0・既存破壊0）。resolver ミューテーション実測＝不在 src で false・実ファイルで true。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run` manifest.test.ts=9 it 全pass（新規4）/ `build`=成功。

**残**: #750（sitemap 逆カバレッジ）は origin/main マージ後 CI 緑回収＆マージ（次イテレーション 1)）。#767（e-Gov 条アンカー）CI 緑回収。O17/T6 実装はオーナー承認待ち。

---

## 2026-07-03 柱C-2 横断検索に治療と仕事の両立支援 病態別6ガイドを収載（seo/cross-search-illness-guides・PR #803）

**背景**: BACKLOG-seo.md の未着手は O17/T6 の Path A 設計ドラフト（オーナー承認待ち）のみで実装タスクなし＝補充指針に従い自領域(横断検索)から新タスクを補充。監査で `/treatment-work-balance/illness-guide/[illness]`（がん/脳卒中/心疾患/糖尿病/メンタルヘルス/難病の6疾患・自己canonical・OGP・PageJsonLd・sitemap収載済みの実在 indexable ページ）が横断検索から**0件**だった発見性の穴を発見。親ハブ `/treatment-work-balance` は FLAGSHIP subItem として feature 収載済みだが、疾患別6ガイドは未収載＝疾患名(「がん 両立支援」等)で個別ガイドへ検索経由で着けなかった（#561 accident 等と同型）。

**変更**: `search-index.ts` に `ILLNESS_CATEGORIES` を動的import する射影ブロックを新設（signs/equipment/faq と同型のブラウザ安全な直import・遅延検索チャンク側にのみ載る）。title=疾患短縮名+両立支援ガイド・subtitle=病態サマリ・keywords=正式疾患名+両立支援頻用語(復職/就業配慮/労務配慮)+病態別リスク(症状語で引ける)。url は `generateStaticParams`(dynamicParams=false)が全 id を解決＝収載集合＝解決集合で必ず着地（幽霊URL 0）。関連法令は条文の権威クエリ汚染回避で keywords へ入れない（保護具/機能と同方針）。目的地ページ扱いで既存 `feature` カテゴリへ収載＝新規タブ/カテゴリ追加なし（6件に比例的）。locked不変条件（就業制限=安衛法61条・石綿事前調査=石綿則3条）は疾患名が当該クエリ非出現で無影響。回帰: `search-index.test.ts` に describe 2本（全6疾患深リンク着地・解決集合一致・幽霊URL0・疾患名クエリ着地）＋既存 feature 件数一致断言を `page-*` 射影源に限定（2源共存に追従）。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run`=全2588 pass（新規2）/ `build`=成功（6ガイド全SSR prerender確認）。再生成データ（docs/rag-metrics-latest.json・chatbot-eval-fresh-results.json）は復元・working tree clean。

**残**: #796（feeds.ts 逆drift ガード）は前イテレーションから CI 進行中＝次イテレーション 1) で緑回収＆マージ。O17/T6 実装はオーナー承認待ち。

---

## 2026-07-04 柱C-2・S 補充 横断検索に助成金・補助金ハブを収載（seo/cross-search-subsidies-hub）

**背景（発見性の穴）**: 横断検索(/search・⌘K)の `feature` カテゴリは FLAGSHIP ナビ由来の目的地ページのみ収載していたため、**sitemap 収載済み・indexable な独立ハブ `/subsidies`（助成金・補助金ガイド）と `/subsidies/calculator`（支給額試算ツール）が FLAGSHIP ナビ非掲載ゆえ横断検索から丸ごと0件**だった。「助成金」「補助金」と打った中小事業主/一人親方（中核ユーザー）が公的助成金ハブへ検索経由で着けない穴。

**是正**: `site-pages-search-source.ts` に `EXTRA_DESTINATION_PAGES`（ナビ外の indexable 目的地ページの手当て補充）を新設し、`getSitePageSearchEntries()` 末尾でパス衝突時ナビ優先（先勝ち）で merge。keyword は個別助成金の金額/条件を写経・捏造せず、ページ metadata description が自ら advertise する3制度（エイジフレンドリー補助金・働き方改革推進支援助成金・建退共）＋一般語に限定。新規タブ/カテゴリ追加なし（既存 `feature` へ2件を比例収載）。

**回帰**: `site-pages-search-source.test.ts` に describe「補充: FLAGSHIP 外の indexable 目的地ページ」4 it＝(1)全補充URLが `sitemap()` に実在＝indexable（薄い/noindex 混入0）(2)ナビ正本と非重複（二重管理回避）(3)助成金ハブが「助成金」「補助金」keyword 付きで結線 (4)全補充URLが最終射影に含まれる。`search-index.test.ts` feature describe に1 it＝「助成金」「補助金」で /subsidies・「助成金 試算」で /subsidies/calculator 着地。既存 feature 件数一致断言は `getSitePageSearchEntries().length` 動的比較で自動追従。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run`=全2602 pass（新規5）/ `build`=成功。再生成データ（docs/rag-metrics-latest.json・chatbot-eval-fresh-results.json）は復元・working tree clean。

**残**: `/subsidies` を FLAGSHIP ナビ subItems へ載せるか否かはナビ config 所有の ux-hub 班判断（要・他班）。個別助成金の詳細ページ `/subsidies/[id]` は未実装。O17/T6 実装はオーナー承認待ち。

---

## 2026-07-04 柱C-2補充 横断検索に対象4類型のペルソナ別ポータル /for/<persona> を収載（PR #812）

**背景**: サイトの唯一のゴールは対象ユーザー4類型（現場監督・一人親方・安全担当・労働安全コンサル）にとって日本一使いやすいこと。その4類型ごとの入口ハブ `/for/{construction,solo,manager,consultant}`（sitemap priority 0.85〜0.9・自己canonical・index:true・OGP付の実在 indexable ポータル）が、横断検索(/search・⌘K)から丸ごと0件だった。「職長」「一人親方」「安全衛生担当」「社労士」と**自分の立場名**で探した当のターゲットユーザーが、自分専用の入口ハブへ検索経由で着けない発見性の穴（#561 等と同型）。

**是正**: FLAGSHIP ナビ非掲載のため `site-pages-search-source.ts` の `EXTRA_DESTINATION_PAGES`（助成金ハブと同じ手当て補充枠）へ4件追加。title/subtitle/keywords は各ページ metadata の title/description が自ら advertise する**立場・役割名のみ**に限定し、墜落/足場/化学物質等のハザード語・法令名は入れない＝条文・通達の権威クエリを汚さない（保護具/機能ページ/疾患ガイドと同方針）。feature 扱いで既存カテゴリへ収載＝新規タブ/カテゴリ追加なし（4件に比例的）。url は全て sitemap 実在・FLAGSHIP ナビと非重複（drift ガードで機械固定＝幽霊URL0・薄い/noindex混入0）。

**回帰**: `site-pages-search-source.test.ts` に「/for/<persona> が立場名 keyword で結線」describe を追加。`search-index.test.ts` feature describe に buildSearchIndex 経由で立場名クエリ（職長/現場代理人/一人親方/安全衛生担当/社労士/労働安全コンサルタント）が当該ポータルへ着地する統合 it を追加。既存 feature 件数一致断言は `getSitePageSearchEntries().length` 動的比較で自動追従。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run`=全2640 pass / `build`=成功。再生成データ（docs/rag-metrics-latest.json・chatbot-eval-fresh-results.json）は復元・working tree clean。

**残**: `/for/<persona>` を FLAGSHIP ナビへ載せるか否かはナビ config 所有の ux-hub 班判断（要・他班）。O17/T6 実装はオーナー承認待ち。

---

## 2026-07-04 柱C-2補充 かな入力→カタカナ表記の橋渡し（foldKana を fuzzyMatch 限定で追加・PR #837）

**課題**: `fuzzy-search.ts` の `normalizeSearchText` は長音符・全角半角・小書き・NFKC を吸収していたが**ひらがな↔カタカナの畳み込みが無く**、モバイルでかな入力する現場ユーザー（現場監督・一人親方）がカタカナ表記の保護具・化学物質・標識・通達（フルハーネス/ベンゼン/クレーン等）を「ふるはーねす」等のひらがなで打つと 0 件になる取り逃しがあった（実測 `fuzzyMatch("ふるはーねす","フルハーネス…")`=false）。

**設計判断（既存破壊0）**: 当初 `normalizeSearchText` 自体へ畳み込みを入れたところ、共有する**チャットボットRAG（`rag-search.ts`）と横断検索スコアリング（`cross-search/score.ts`）で法令コーパスのかな畳み込みによりランキングが劣化**し回帰した（実測 RAG fresh 0.90→0.86・Citation@1 0.65→0.59・Recall@5 0.99→0.89）。→ 畳み込みを純関数 `foldKana`（U+3041〜U+3096 を +0x60 シフト）へ切り出し、**UIの部分一致マッチ `fuzzyMatch`/`fuzzyMatchAll` でのみ**クエリ・ターゲット双方に**対称適用**する形へ限定。`normalizeSearchText` は byte-identical に保ち RAG/横断検索スコアは無影響。対称適用ゆえ recall を広げるだけで false negative は生まない。消費側（`accident-database-panel`/`law-revision-list` の部分一致UI）は import 変更なしで自動的にかな入力へ対応。

**回帰**: `fuzzy-search.test.ts` に (1) `foldKana` 単体（ひらがな→カタカナ・カタカナ/漢字/英数は不変）(2) `fuzzyMatch`/`fuzzyMatchAll` のかな入力ヒット（順逆対称・複数トークンAND）(3) `normalizeSearchText` がかな畳み込みしない前提固定（＝RAG回帰の再発を機械封止）を追加。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run`=全2689 pass（1 skip）/ `build`=成功。再生成データ（docs/rag-metrics-latest.json・chatbot-eval-fresh-results.json）は `generated_at` timestamp のみ差分＝correct 件数不変を確認し復元・working tree clean。

**残**: 横断検索スコアリング（cross-search）自体はかな読みを law-alias で吸収済みのため今回対象外。O17/T6 実装はオーナー承認待ち。

---

## 2026-07-04 柱C-3-3・S 補充 — sitemap-index.xml 逆カバレッジガード新設（孤立子サイトマップの機械検知）

**背景（発見性の穴）**: sitemap-index.xml/route.test.ts の既存 forward ガードは index が列挙する子サイトマップの面子・順序・lastmod を固定していたが、逆向き「実在する子 route → index への登録」が皆無だった。当班/他班が新しい子サイトマップ `src/app/sitemap-<slug>.xml/route.ts` を新設しても sitemap-index.xml/route.ts の `children` へ追加し忘れると、その子は robots.ts の `sitemap:` が指す index から一切参照されず、**クローラの入口から到達不能な孤立サイトマップ**のまま放置される。子サイトマップ1本は数千URL（化学物質は約3,515物質）を束ねるため、登録漏れ1本＝その配下ページ群がまるごと発見されない（#750 の sitemap 逆カバレッジ・#774 の RSS フィード逆カバレッジと同型だが影響は桁違いに大きい）。

**変更**: `sitemap-index.xml/route.test.ts` に逆カバレッジ describe（2 it・**コード変更0の純ガード**）を追加。`readdirSync` で `src/app` 直下を走査し、GET ハンドラを持つ `sitemap-<slug>.xml/route.ts`（index 自身は除外）を全て `/sitemap-<slug>.xml` へ射影し、各々が index 出力の `<loc>` に登録済みであることを必須化（走査サニティ＝既知5本以上）。

**回帰（ミューテーション実測）**: sitemap-chemicals を index children から外すと当該 it が `[ '/sitemap-chemicals.xml' ]` で赤化することを確認。現状の実在5子サイトマップ（記事/事故/通達/保護具/化学物質）は全登録済み＝コード修正不要。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run`=全2645 pass（新規2含む）/ `build`=成功。再生成データ（docs/rag-metrics-latest.json・chatbot-eval-fresh-results.json）は復元・working tree clean。

**残**: なし（純ガード・要・他班なし）。今イテレーション冒頭で衝突 PR #774/#780/#786 に origin/main を通常マージして再push（CI再走中）。

---

## 2026-07-04 柱C-2補充 横断検索に独立ハブ2件（新着情報/whats-new・記録キット/site-records）を収載 (PR #849)

**背景（発見性の穴）**: 目的地ページ射影 `EXTRA_DESTINATION_PAGES`（FLAGSHIP 非掲載だが sitemap 収載済み・indexable な独立ハブを手当てで載せる当班機構）は助成金ハブ＋ペルソナ別ポータル4類型までで、2つの高検索意図ハブが横断検索(/search・⌘K)から**丸ごと0件**だった。`/whats-new`（法改正・労災速報・通達・報道を新着順に一元表示する daily 更新ハブ＝最新情報の入口）は「新着」「更新情報」「速報」で0件、`/site-records`（受入教育記録・KY・打合せ書・WBGT記録簿等を束ねる現場記録キット。配下ツールはナビ掲載でもハブ自体は非掲載）は「記録キット」「安全記録」「帳票」で0件。機能名やトピック名では引けても**ハブを名指す語**で着けなかった。

**是正**: `EXTRA_DESTINATION_PAGES` に2件を追加。keyword は各ページ metadata（title/description）が advertise するハブ概念語のみに絞り（新着情報/新着/更新情報/最新情報/速報/一元表示/RSS・記録キット/現場記録/安全記録/記録簿/帳票/現場帳票）、「法改正」「通達」の裸クエリや KY/WBGT 等の個別ツール名は入れない＝revision/notice レコード・個別ツール自ページ解決を汚さない（ペルソナ/機能ページと同方針）。feature 扱いで既存カテゴリへ収載＝新規タブ0。

**テスト**: `site-pages-search-source.test.ts` に「独立ハブがハブ概念語 keyword で結線」it 追加（/whats-new=新着情報/更新情報・/site-records=記録キット/安全記録 を機械固定）。既存 drift ガード（全 url が sitemap 実在＝幽霊URL0・薄い/noindex 混入0、FLAGSHIP 非重複、最終射影包含）が両 url を自動検証。

**ゲート**: `tsc --noEmit`=0 / `lint`=errors0（warnは既存の別ファイル）/ `vitest run`=全2721 pass（新規1含む）/ `build`=成功。再生成データ（docs/rag-metrics-latest.json・chatbot-eval-fresh-results.json）は復元・working tree clean。

**残**: なし（要・他班なし）。前イテレーションの教育コース PR #845 は CI（e2e/smoke）走行中＝次イテレーション冒頭で回収。
