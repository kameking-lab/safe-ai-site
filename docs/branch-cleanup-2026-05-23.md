...
- A分類 (即削除可): 73 件 — **環境制約により本PRでは削除未実行。ローカル実行スクリプト同梱**
- B分類 (削除可、社長確認待ち): 85 件
- C分類 (要確認、60日以上): 0 件
- D分類 (削除不可、要判断): 11 件
...
本リモート実行環境の git proxy がブランチ削除 (`git push origin --delete`) を HTTP 403 で拒否するため、A分類73件の自動削除は実行できませんでした。ローカル環境で以下のスクリプトを実行してください:

```bash
cd safe-ai-site
git checkout chore/stale-pr-cleanup-2026-05-23
git pull
bash scripts/delete-A-branches-2026-05-23.sh
```

スクリプトは `scripts/delete-A-branches-2026-05-23.sh` に同梱、73件のブランチを順次 `git push origin --delete` します。

B分類削除をご承認頂ければ同様のスクリプトを追加生成可能です。
...
## A分類 — 即削除可 (73 件、ローカル実行待ち)

条件: `claude/*` プレフィックス + 対応 merged PR あり。本PRでは環境制約で削除未実行。`scripts/delete-A-branches-2026-05-23.sh` を **ローカルから** 実行してください。
...
- 削除前: 174 (本PR ブランチ追加で 175)
- A削除実行後: 175 - 73 = 102
- B削除承認 → 実行後 (想定): 102 - 85 = 17
- 最終残数想定 (B承認時): D 11 + Open PR head 4 + main + 作業ブランチ1 = 17
