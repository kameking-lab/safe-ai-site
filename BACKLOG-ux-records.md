...
- [x] 【柱3レビュー】安全日誌(/safety-diary 打合せ書)を「毎日書く職長」ペルソナで実機レビュー→当日の記入状態(下書き/保存済み)が無読で読めるか点検。欠落: 結論カードが incomplete/complete の2状態のみで、**記入完了後に「もう保存一覧に保存したか」が一目で分からなかった**（complete は保存有無に関わらず緑「記入完了」＝下書きと保存済みが同じ見た目／保存状態は下部バーの極小文字「自動保存:HH:MM」のみ、しかも自動保存は端末内下書きで保存一覧には未反映＝誤認の元）。computeMeetingPaperStatus に第3状態 saved を追加=緑「保存済み→保存一覧で確認」、記入完了・未保存は青「保存する」に整理（緑=保存済みに限定）。saved判定は store.savedAt が自動保存/翌日複製でも更新され信頼できないため、手動「保存」した内容とのJSON一致でセッション内厳密追跡（誤って緑にしない）。下部バー文言と自動保存ラベルも「下書き自動保存／保存一覧に保存済み」に是正。A4印刷シートは print:hidden で不変。共通基盤(paper-status)は足すだけ。vitest 8/8＋Playwright無読9/9合格(4状態+保存後編集で緑が外れる回帰)。(2026-06-14, PR未確定)
