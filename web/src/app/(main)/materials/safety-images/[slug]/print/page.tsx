...
import { getSafetyImageTheme, SAFETY_IMAGE_THEMES } from "@/data/safety-image-library";
...
export default async function SafetyImagePrintPage({ params }: PageProps) {
  ...
  if (!theme) notFound();
  const printOrientation = theme.orientation === "portrait" ? "portrait" : "landscape";
  const outputSize = defaultOutputSize(theme.recommendedSize, theme.orientation);
  const printMessage = resolveSafetyImageMessage(theme, {
    mode: "default",
    language: "ja",
    text: theme.texts.ja,
    subMessage: "",
  ...
  const pdfHref = `/api/safety-images/${theme.slug}/download?mode=default&lang=ja&brand=branded&size=${outputSize}&format=pdf`;
  return (
    <div className="mx-auto max-w-5xl bg-white p-4 text-slate-950 sm:p-8">
      <style>{`@page { size: A4 ${printOrientation}; margin: 0; } @media print { .print-controls { display: none !important; } body { background: white !important; } .print-sheet { width: 100vw !important; height: 100vh !important; border: 0 !important; box-shadow: none !important; } }`}</style>
      ...
      <article className={`print-sheet relative mx-auto overflow-hidden border border-slate-300 bg-[#eef7f7] shadow-lg ${theme.orientation === "portrait" ? "aspect-[1/1.414] max-h-[calc(100vh-2rem)]" : theme.orientation === "square" ? "aspect-square" : "aspect-[1.414/1] max-w-full"}`} aria-label={`${theme.title}の印刷用安全看板`}>
        <Image src={theme.originalPath} alt={`${theme.title}の文字なしオリジナルイラスト`} fill priority sizes="100vw" className="object-contain" />
