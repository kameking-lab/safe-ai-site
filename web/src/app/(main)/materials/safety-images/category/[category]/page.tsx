...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const category = getSafetyImageCategory(categoryId);
  if (!category) return {};
  const path = `${SAFETY_IMAGE_LIBRARY_PATH}/category/${category.id}`;
  const title = `${category.label}｜無料の安全画像・イラスト`;
  const count = SAFETY_IMAGE_THEMES.filter((theme) => theme.category === category.id).length;
  const description = `${category.description} 安全AIポータル新規制作の${count}点を、文字編集・5言語・A4/A3・JPEG/PDFで利用できます。`;
  ...
  return {
    ...
    openGraph: withSiteOpenGraph(path, {
      ...
      type: "website",
      images: first
        ? [{ url: first.previewPath, width: 720, height: first.orientation === "portrait" ? 899 : 480, alt: `${first.title}の安全イラスト` }]
        : undefined,
    }),
...
export default async function SafetyImageCategoryPage({ params }: PageProps) {
  ...
  const path = `${SAFETY_IMAGE_LIBRARY_PATH}/category/${category.id}`;
  const description = `${category.description} ${themes.length}点を公開しています。`;
  return (
    <div className="pb-16">
      <PageJsonLd name={category.label} description={description} path={path} />
