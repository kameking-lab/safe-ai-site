...
import { Building2, FileText, ShieldAlert, ListChecks, ExternalLink } from "lucide-react";
...
export function IndustryRiskClient() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h3 className="text-base font-bold text-slate-900">標準対策</h3>
        <ul className="mt-3 space-y-1 text-sm leading-6 text-slate-800">
      ...
      <section className="rounded-2xl border border-rose-200 bg-rose-50/60 p-5 shadow-sm">
        ...
        <ul className="mt-3 space-y-1 text-sm leading-6 text-rose-900">
          {rule.r7ComplianceFocus.map((t) => (
            <li key={t} className="flex gap-2">
              <span className="mt-1 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-rose-600" />
              {t}
            </li>
          ))}
        </ul>
      ...
      <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
        <h3 className="text-base font-bold text-slate-900">関連法令・指針</h3>
        <ul className="mt-3 space-y-1 text-sm leading-6 text-slate-800">
