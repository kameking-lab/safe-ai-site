...
export async function POST(request: Request) {
  ...
  if (!apiKey || apiKey === "dummy") {
    ...
    const sources: ChatbotSource[] = articles.map((a) => ({
    ...
    const fallbackAnswer = articles.length > 0
      ? `AIによる回答生成は現在利用できませんが、関連する法令条文が見つかりました。下記の参照条文をご確認ください。\n\nGEMINI_API_KEYを環境変数に設定すると、AIによる詳細な回答が利用できます。`
    ...
    return NextResponse.json<ChatbotResponse>(
      {
        ...
        sources,
        source_type: articles.length > 0 ? "rag" : "ai_inference",
