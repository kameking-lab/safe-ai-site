...
import { CAS_LAW_INDEX } from "./cas-law-index";
...
describe("cas-law-index の自己検査（号参照とスナップショットの一致）", () => {
  ...
  it("nameKeys（CASレス告示名・群指定の名称解決キー）が索引全体で一意", () => {
    // 一窓化(2026-07-11): 名称解決の衝突は誤った法令結論に直結するため重複を禁止
    const seen = new Map<string, string>();
    for (const e of CAS_LAW_INDEX) {
      for (const k of e.nameKeys ?? []) {
        expect(seen.has(k), `nameKey重複: "${k}" (${seen.get(k)} と ${e.cas})`).toBe(false);
        seen.set(k, e.cas);
      }
    }
    // 受入アンカー: 溶接ヒューム（CASレス）が名称解決可能であること
    expect(seen.has("溶接ヒューム")).toBe(true);
  });
});
