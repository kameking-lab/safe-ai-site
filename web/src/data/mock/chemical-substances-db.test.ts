 ...
 *
 * 2026-07-03 の欠落是正（BACKLOG-data O1-b）を追加固定する（偽陽性とは別軸＝取りこぼし）:
 *   - 一酸化炭素（第3号2）に特化則3類／硫化水素（第2号35）に特化則2類 を追加
 *   - スチレン(22の2)/インジウム(3の2)/ナフタレン(23の2)/RCF(34の3)/エチルベンゼン(3の3)/
 *     コバルト(13の2) に特別管理物質を追加（特化則38条の4が列挙する第2号レンジに該当）
 *   - 一酸化炭素(第3号2)・硫化水素(第2号35)は38条の4レンジ外＝特別管理物質でないことも固定
 *   特化則38条の4（作業の記録・30年保存）が定める特別管理物質＝令別表第3第一類物質（塩素化ビフェニル
 *   等を除く）＋第2号「3の2〜6、8、8の2、11〜12、13の2〜15の2、18の2〜19の5、21、22の2〜22の5、
 *   23の2〜24、26、27の2、29、30、31の2、32、33の2、34の3」に掲げる物（e-Gov 2026-07-03取得）。
 */
...
const EXPECTED_TOKKA_CLASS: Record<string, { category: ChemicalCategory; go: string }> = {
  ...
  // 2026-07-03 追加是正（O1-b・欠落是正）
  "630-08-0": { category: "特化則3類", go: "第3号2 一酸化炭素" },
  "7783-06-4": { category: "特化則2類", go: "第2号35 硫化水素" },
};
...
const IS_SPECIAL_CONTROL_CAS = new Set<string>([
  ...
  "75-01-4", // 塩化ビニル(6)
  "8007-45-2", // コールタール(14)
  // 2026-07-03 追加是正（O1-b・欠落是正）— いずれも38条の4の第2号レンジに該当
  "100-42-5", // スチレン(22の2)
  "7440-74-6", // インジウム化合物(3の2)
  "91-20-3", // ナフタレン(23の2)
  "142844-00-6", // リフラクトリーセラミックファイバー(34の3)
  "100-41-4", // エチルベンゼン(3の3)
  "7440-48-4", // コバルト及びその無機化合物(13の2)
]);
const NOT_SPECIAL_CONTROL_CAS = new Set<string>([
  "7440-43-9", // カドミウム(10) — 38条の4の号に非該当
  "1332-21-4", // 石綿 — 特化則外
  "106-99-0", // 1,3-ブタジエン — 令別表第3非収載
  "630-08-0", // 一酸化炭素 — 第三類物質(第3号2)は38条の4レンジ外
  "7783-06-4", // 硫化水素 — 第2号35は38条の4レンジ（末尾34の3）外
]);
