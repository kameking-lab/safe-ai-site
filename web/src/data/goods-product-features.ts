export type GoodsProductFeature = {
  ...
  /**
   * 商品名にいずれかの語が無い出品は表示しない（使用区分を明記した出品に限る）。
   * 規格適合の保証ではなく、区分を名乗らない防災用品等の混入を防ぐための条件。
   */
...
export const GOODS_PRODUCT_FEATURES: Readonly<Record<string, readonly GoodsProductFeature[]>> = {
  "head-protection": [
    { id: "flying", label: "飛来・落下物", detail: "資材や工具が頭に当たるおそれ", searchQuery: "保護帽 飛来 落下", nameMustInclude: ["飛来", "落下物"], check: "型式の使用区分と飛来・落下物への適合をメーカー資料で確認" },
    { id: "fall", label: "墜落時の頭部保護", detail: "高所作業や足場で使う", searchQuery: "保護帽 墜落時保護", nameMustInclude: ["墜落"], check: "墜落時保護の使用区分とあごひもの仕様をメーカー資料で確認" },
