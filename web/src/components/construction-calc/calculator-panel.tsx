...
import { consumeConstructionCalcHandoff } from "@/lib/construction-calc/transient-handoff";

/**
 * 建設計算 1計算機1画面のインタラクティブ部（入力→結論→計算過程→定型説明）。
 *
...
export function CalculatorPanel({ slug }: { slug: string }) {
  ...
  // 自由記述から読み取った現場条件は、URLやブラウザ保存領域へ出さず、
  // 同一タブの一回限りメモリ引継ぎから反映する。
  // 既存ブックマークとの互換性のため旧クエリは読取専用で残すが、新規生成はしない。
  useEffect(() => {
    if (!calc) return;
    const transient = consumeConstructionCalcHandoff(slug);
    const overrides: RawValues = {};
    if (transient) {
      for (const f of calc.fields) {
        const value = transient.values[f.id];
        if (
          (typeof value === "string" || typeof value === "number") &&
          String(value) !== ""
        ) {
          overrides[f.id] = String(value);
        }
      }
    } else {
      const legacyParams = new URLSearchParams(window.location.search);
      for (const f of calc.fields) {
        const value = legacyParams.get(f.id);
        if (value !== null && value !== "") overrides[f.id] = value;
      }
    }
  ...
  const fetchExplanation = async () => {
    ...
    try {
      const res = await fetch("/api/construction-calc", {
        ...
        body: JSON.stringify({ slug: calc.slug, values }),
      });
      ...
      setAiText(
        "計算結果の説明を取得できませんでした。計算結果と計算過程を確認してください。",
      );
    } finally {
      setAiLoading(false);
    }
      ...
      <div className="space-y-4 print:hidden">
        ...
        >
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
            ...
            {outcome.tone === "safe" && (
              <Mascot
                ...
                size="md"
                alt=""
                className="order-last ml-auto hidden shrink-0 sm:block"
              />
            )}
            ...
            {outcome.items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <dd
                    className={`shrink-0 font-bold ${itemTone ? itemTone.text : "text-slate-900 dark:text-white"}`}
                  >
                    {item.value}
                  </dd>
                </div>
          ...
          {outcome.warnings.length > 0 && (
            <ul
              className="mt-4 space-y-1 rounded-xl border border-amber-300 bg-amber-50/80 p-3 text-xs leading-5 text-amber-900 dark:border-amber-700 dark:bg-amber-950/30 dark:text-amber-200"
              aria-label="この条件での注意"
            >
          ...
          {/* 決定論的な説明＋計算書出力（提出用A4帳票を registry から自動生成して印刷/PDF化） */}
          <div className="mt-4">
