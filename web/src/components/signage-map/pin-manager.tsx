...
export function PinManager({
  ...
  const canSubmit =
    !!pendingCoords && !!label.trim() && !reachedLimit && !submitting;
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "ピンの保存に失敗しました。",
      );
    } finally {
      setSubmitting(false);
    }
    ...
    <section className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-slate-100">
          ピン管理（{pins.length}/{limit}）
        </h3>
      </div>

      <form
        onSubmit={submit}
        className="space-y-2 rounded-lg border border-slate-700 bg-slate-950/50 p-3"
      >
        <p className="text-[10px] text-slate-400">
          地図を{" "}
          <span className="font-bold text-slate-200">右クリック / 長押し</span>{" "}
          すると、その地点が候補になります。
        </p>
        <p className="rounded border border-amber-700 bg-amber-950/40 px-2 py-1.5 text-[11px] leading-5 text-amber-200">
          ピンはこの端末のブラウザ内だけに保存されます。サーバーへ送信せず、
          警報メールや通知は行いません。
        </p>
        {pendingCoords ? (
          ...
          <p className="rounded bg-slate-900/80 px-2 py-1 text-[10px] text-slate-400">
            候補地点が未選択です。
          </p>
        )}
        ...
        <button
          type="submit"
          disabled={!canSubmit}
          className="flex min-h-[44px] w-full items-center justify-center rounded bg-emerald-700 px-3 text-xs font-bold text-white hover:bg-emerald-800 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-300"
        >
        ...
        {pins.map((p) => (
          ...
          >
            ...
            >
              ...
              <p className="text-[10px] text-amber-300">端末内保存・通知なし</p>
            </button>
            <button
              type="button"
