...
export function HomeSafetyUpdates({
  ...
  return (
    <section
      ...
      className="home-render-skip-section home-skip-updates scroll-mt-24 bg-slate-950 px-4 py-5 text-white sm:py-9"
        ...
        <div className="mt-3 grid gap-3 sm:mt-4 sm:gap-4 lg:grid-cols-2">
                ...
                <p className="text-[11px] font-black leading-5 text-rose-100">
                  ...
                  <time dateTime={featuredAccident.publishedAt}>
                    {dateFormatter.format(
                      new Date(featuredAccident.publishedAt),
                    )}
                  </time>{" "}
                  ...
                  >
                    <p className="text-[11px] font-black text-rose-200">
                      ...
                      <time dateTime={additionalAccident.publishedAt}>
                        {dateFormatter.format(
                          new Date(additionalAccident.publishedAt),
                        )}
                      </time>
                    </p>
                    <a
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span
                data-status-badge
                className="rounded-full bg-violet-950 px-2 py-1 text-[11px] font-black text-white"
              >
                {reform.status}
              </span>
            </div>
            ...
            <div className="mt-2 grid grid-cols-2 gap-2 sm:mt-3">
                ...
                <ul className="border-t border-slate-300 py-2">
                  {HOME_ADDITIONAL_LAW_REFORMS.map((item) => (
                    ...
                    >
                      ...
                      <p className="mt-1 text-xs leading-5">{item.action}</p>
                      <a
                        href={item.sourceUrl}
                        target="_blank"
                        rel="noopener noreferrer"
              ...
              <Link
                href="/laws"
                className="inline-flex min-h-11 items-center justify-center text-xs font-black text-violet-950 underline underline-offset-4 sm:text-sm"
              >
                法改正一覧を見る
              </Link>
            </div>
