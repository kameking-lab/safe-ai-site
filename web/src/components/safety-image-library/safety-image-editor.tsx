...
import {
  ...
  type SafetyImageMessageSegment,
...
export function SafetyImageEditor({ theme }: { theme: SafetyImageTheme }) {
  ...
  const language = languages[0] ?? "ja";
  const renderLanguage = languages.includes("ja")
    ? "ja"
  ...
  const displaySegments = resolveSafetyImageMessageSegments(theme, {
    mode: "edited",
  ...
  function toggleLanguage(nextLanguage: SafetyImageLanguage, checked: boolean) {
    const selected = checked
      ? LANGUAGE_OPTIONS.map(([value]) => value).filter(
          (value) => value === nextLanguage || languages.includes(value),
        )
      : languages.filter((value) => value !== nextLanguage);
    if (selected.length === 0) return;
    setLanguages(selected);
    const nextPrimary = selected[0];
    setNumericUnit(theme.numericTemplate?.units[nextPrimary] ?? "");
    if (selected.length > 1 || nextPrimary !== "ja")
      setWritingMode("horizontal");
  }
  ...
  async function downloadCustomized() {
    ...
    try {
      ...
      anchor.download = `${theme.slug}-${languages.map((value) => value.toLowerCase()).join("-")}-${outputSize}.${extension}`;
      anchor.click();
  ...
  return (
    <>
      <div data-safety-sign-editor>
            ...
            <div className="min-w-0 xl:sticky xl:top-24 xl:self-start">
              ...
              >
                ...
                {displayText ? (
                  <span id={previewDescriptionId} className="sr-only">
                    {displaySegments.map((segment, index) => (
                      ...
                      >
                        {SAFETY_IMAGE_LANGUAGE_LABELS[segment.language]}:{" "}
                        {segment.text}。
                      </span>
                    ))}
                  </span>
                ...
                {displayText && !previewFit ? (
                  <p
                    role="alert"
                    className="absolute inset-x-[5%] top-1/2 z-20 -translate-y-1/2 rounded-xl border-2 border-red-700 bg-white p-3 text-center text-sm font-black text-red-800"
                  >
        ...
        <section
          className="mt-8 rounded-3xl border border-emerald-200 bg-emerald-50 p-4 sm:p-6 dark:border-emerald-900 dark:bg-emerald-950"
          aria-labelledby="download-heading"
        >
          <h2
            id="download-heading"
            className="text-2xl font-black text-emerald-950 dark:text-emerald-100"
          >
            ダウンロード
          </h2>
          ...
          <p
            role="status"
            aria-live="polite"
            className="mt-3 text-sm font-black text-red-700 dark:text-red-300"
          >
            {error}
          </p>
      ...
      <noscript>
        ...
        <section className="mt-7 rounded-2xl border border-amber-300 bg-amber-50 p-5 text-slate-900">
          <h2 className="text-xl font-black">
            JavaScriptを有効にすると文字を編集できます
          </h2>
          <p className="mt-2 font-bold leading-7">
            文字なし画像は通常リンクから取得できます。入力内容はURLへ含めません。
          </p>
          <a
            href={`${downloadEndpoint}?mode=clean&lang=ja&brand=none&size=${recommendedOutputSize}&format=png`}
            className="mt-4 inline-flex min-h-11 items-center rounded-lg bg-emerald-800 px-4 font-black text-white"
          >
            文字なしPNGをダウンロード
          </a>
        </section>
...
function SafetyImageTextPreview({
  ...
  align: TextAlign;
  lineHeight: number;
  segments: SafetyImageMessageSegment[];
  ...
  if (fit.kind === "vertical") {
    ...
    return (
      <svg
        ...
        role="img"
        aria-label={`文字編集プレビュー: ${segments.map((segment) => SAFETY_IMAGE_LANGUAGE_LABELS[segment.language]).join("・")}`}
      ...
      >
        {band ? (
          <rect
            ...
            strokeWidth={
              border ? Math.max(3, Math.round(dimensions.width * 0.002)) : 0
            }
          />
        ) : null}
        {fit.columns.flatMap((column, columnIndex) =>
          column.map((character, characterIndex) => {
            const x =
              panelX +
              fit.verticalPanelWidth -
              fit.panelPadding -
              fit.fontSize * 0.55 -
              columnIndex * fit.columnGap;
            const y =
              panelY +
              fit.panelPadding +
              fit.fontSize * 0.88 +
              characterIndex * fit.fontSize * lineHeight;
            return (
              <text
                ...
                textAnchor="middle"
                lang={language}
                style={{ fontFamily: fontFamilyForLanguage(language) }}
                {...commonText}
              >
                {character}
              </text>
  ...
  const x =
    align === "left"
      ...
      : align === "right"
        ...
        : dimensions.width / 2;
  const localizedLines = segments.flatMap((segment) =>
    wrapSafetyImageText(
      segment.text,
      (fit.panelWidth - fit.panelPadding * 2) / fit.fontSize,
    ).map((line) => ({ line, language: segment.language })),
  );
  const renderedLines =
    localizedLines.length === fit.lines.length
      ? localizedLines
      : fit.lines.map((line) => ({ line, language }));
  return (
    <svg
      className="pointer-events-none absolute inset-0 z-10 h-full w-full"
      viewBox={`0 0 ${dimensions.width} ${dimensions.height}`}
    ...
    >
      ...
      {renderedLines.map(({ line, language: lineLanguage }, index) => (
        <text
          key={`${index}-${line}`}
          x={x}
          y={
            ...
            fit.panelPadding +
            fit.fontSize * 0.87 +
            index * fit.fontSize * lineHeight
          }
          textAnchor={textAnchor}
          lang={lineLanguage}
          style={{ fontFamily: fontFamilyForLanguage(lineLanguage) }}
          {...commonText}
        >
          {line || "\u00a0"}
        </text>
