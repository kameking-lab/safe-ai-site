...
const LINKS = [
  ...
  },
  {
    group: "学ぶ",
    items: [
      { href: "/e-learning", label: "Eラーニング", sub: "テーマ別学習コンテンツ", color: "bg-slate-100 text-slate-800" },
      { href: "/exam-quiz", label: "過去問クイズ", sub: "労働安全コンサルタント試験", color: "bg-amber-500 text-white" },
    ],
  },
  {
    group: "調べる",
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/goods", label: "安全グッズ", sub: "推奨装備・用品", color: "bg-slate-100 text-slate-800" },
    ],
  },
] as const;
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {group.items.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`rounded-lg px-3 py-2.5 ${item.color} flex flex-col transition hover:opacity-90`}
                >
                  <span className="text-xs font-bold leading-tight">{item.label}</span>
                  <span
                    className={`mt-0.5 text-[10px] leading-tight ${
                      item.color.includes("text-white") ? "text-white/70" : "text-slate-500"
                    }`}
                  >
                    {item.sub}
                  </span>
