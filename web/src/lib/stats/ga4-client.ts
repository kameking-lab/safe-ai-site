...
function isGa4Configured(): boolean {
      console.log("[stats:debug] GA4_PROPERTY_ID:", process.env.GA4_PROPERTY_ID ? `set (length=${process.env.GA4_PROPERTY_ID.length})` : "undefined");
      console.log("[stats:debug] GOOGLE_APPLICATION_CREDENTIALS_JSON:", process.env.GOOGLE_APPLICATION_CREDENTIALS_JSON ? `set (length=${process.env.GOOGLE_APPLICATION_CREDENTIALS_JSON.length})` : "undefined");
      console.log("[stats:debug] GOOGLE_APPLICATION_CREDENTIALS:", process.env.GOOGLE_APPLICATION_CREDENTIALS ? `set (length=${process.env.GOOGLE_APPLICATION_CREDENTIALS.length})` : "undefined");
      console.log("[stats:debug] env keys (GA4/GOOGLE):", Object.keys(process.env).filter(k => k.startsWith("GA4") || k.startsWith("GOOGLE")));
...
export async function fetchStats(period: StatsPeriod): Promise<StatsResponse> {
  ...
  try {
          ...
          return {
                    ...
                    _debug: {
                                ...debugInfo,
                                ga4_fetch_error: error instanceof Error ? error.message : String(error),
                    },
          } as StatsResponse & { _debug: typeof debugInfo & { ga4_fetch_error: string } };
  }
}
