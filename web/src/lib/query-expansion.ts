...
const EXPANSION_RULES: ExpansionRule[] = [
  ...
  // 「足場」だけで資格・作業主任者へ飛ばすと、手すり・作業床の質問まで
  // 教育条文が乗っ取る。組立て等又は資格意図が明示された場合だけ教育語を足す。
  {
    pattern:
      /(?:足場[^。！？?]{0,24}(?:組立|解体|変更|資格|教育|講習|作業主任者)|(?:組立|解体|変更|資格|教育|講習|作業主任者)[^。！？?]{0,24}足場)/,
    expansions: ["特別教育", "技能講習", "作業主任者"],
  },
  { pattern: /足場/, expansions: ["作業床", "墜落防止設備"] },
  // 電気工事士法上の資格と、安衛法上の特別教育は別制度。ここでは安衛法RAGが
  // 回収すべき教育根拠だけを展開し、免許・作業主任者へは推測展開しない。
  {
    pattern:
      /(?=.*(?:電気作業|電気工事|配線(?:工事|作業)?|活線|充電(?:部|電路)|電路))(?=.*(?:資格|教育|特教|講習))/,
    expansions: ["安全衛生教育", "特別教育", "安衛法第59条", "安衛則第36条", "充電電路"],
  },
  {
    pattern: /活線|充電部|充電電路|電路の近接/,
    expansions: ["感電防止", "充電電路", "電気工事"],
  },
  { pattern: /高所/, expansions: ["墜落制止用器具", "墜落防止"] },
  { pattern: /玉掛け/, expansions: ["技能講習", "特別教育"] },
  // 「就業制限」は速度・用途外使用・検査等の運用質問を資格質問へ変えて
  // しまうため展開しない。資格意図は元質問を見てrag-search側で分岐する。
  { pattern: /フォークリフト/, expansions: ["技能講習", "特別教育"] },
