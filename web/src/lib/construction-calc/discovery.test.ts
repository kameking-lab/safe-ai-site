...
describe("construction-calc 発見層: law → calc 逆リンク", () => {
  ...
  it("安衛則518条・519条 → 安全ネット基準チェック（防網の代替措置根拠。539条=保護帽は無関係）", () => {
    // 部隊4の safety-net-check.ts basis が根拠とする一次条文（作業床設置困難時の防網代替）に一致。
    for (const art of ["第518条", "第519条"] as const) {
      const hits = relatedCalculatorsForArticle("安衛則", art);
      expect(hits.map((c) => c.slug)).toContain("safety-net-check");
    }
    // 第539条（保護帽の着用）は防網と無関係＝安全ネット計算機を引かない。
    expect(
      relatedCalculatorsForArticle("安衛則", "第539条").map((c) => c.slug),
    ).not.toContain("safety-net-check");
  });
  ...
  it("逆リンクが指す条文はすべて law-navi に実在（curated で解決＝幽霊リンク0）", () => {
    ...
    // 代表条（クレーン則213・安衛則518・356）は permalink 生成集合に実在することを固定する。
