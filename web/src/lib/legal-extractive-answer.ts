...
function compact(value: string): string {
  return normalizeLegalConversationText(value).replace(/[\s　、。,!?！？]/g, "");
}
...
function explicitlyRequestedItemLabel(query: string): string | undefined {
  ...
  const branches = [...(match[2] ?? "").matchAll(/の\s*([0-9一二三四五六七八九十百千]+)/g)]
...
export function explicitlyRequestedItemLabels(
  ...
  const normalized = explicitItemQueryScope(query, articleNum).normalize("NFKC");
  const labels: string[] = [];
  ...
  for (const range of normalized.matchAll(
    ...
    if (start === null || end === null || start > end || end - start > 100) continue;
  ...
  for (const item of normalized.matchAll(
    ...
    const branches = [...(item[2] ?? "").matchAll(
      /の\s*([0-9一二三四五六七八九十百千]+)/g,
...
function explicitArticleLocation(
  ...
  while (offset >= 0) {
    ...
    const isLongerArticle =
      !core.includes("条の") && /^の[0-9一二三四五六七八九十百千]+/u.test(suffix);
...
function explicitUnitQueryScope(query: string, articleNum: string): string {
  ...
  const nextArticle = normalized
    .slice(afterCurrent)
    .search(/第[0-9一二三四五六七八九十百千]+条(?:の[0-9一二三四五六七八九十百千]+)?/u);
...
function isDefinitelyAtLeast(load: LoadRange | null, threshold: number): boolean {
  return Boolean(
    load &&
...
export function expandVerifiedLegalEvidenceArticles(
  ...
  if (
    /玉掛/.test(normalized) &&
    /資格|免許|技能講習|特別教育|何(?:t|トン)から|必要|いる|要る/i.test(normalized)
  ) {
    if (isDefinitelyBelow(load, 1)) {
  ...
  if (/移動式クレーン/.test(normalized) && /資格|免許|技能講習|特別教育/.test(normalized)) {
    if (isDefinitelyBelow(load, 1)) {
  ...
  if (/(?:酸欠|酸素欠乏|酸素濃度)/.test(normalized)) {
    ...
    if (/(?:換気|酸素濃度|硫化水素|H2S|ppm|何パーセント|何%)/i.test(normalized)) {
      required.push(["酸欠則", "第2条"], ["酸欠則", "第5条"]);
    }
    if (/(?:換気.*(?:例外|できない|困難)|保護具|空気呼吸器|酸素呼吸器|送気マスク)/.test(normalized)) {
...
export function legalProvisionUnitForQuery(
  ...
  const explicitUnitScope = directlyRequested
    ...
    : normalized;
  const paragraphMatch = explicitUnitScope
    .match(/第?\s*([0-9一二三四五六七八九十百千]+)\s*項(?!目)/);
  const paragraphNumber = paragraphMatch?.[1]
    ? Number(
        /^\d+$/.test(paragraphMatch[1])
          ? paragraphMatch[1]
  ...
  const organicHealthScopeIntent =
    /(?:対象業務|どの業務|誰が対象|対象者)/.test(normalized);
  ...
  if (
    ...
    const tankOutside =
      /(?:タンク等?の?外|タンク外|内部以外|それ以外の屋内)/.test(
        normalized,
      );
    ...
    if (
      ...
      if (/(?:吹付け以外|吹き?付け(?:作業)?ではない|非吹付け)/.test(normalized)) {
...
export function citedLegalAnswerArticles(
  ...
  const indexes = markedIndexes.length > 0
    ? markedIndexes
    : articles.length > 0
      ? [0]
      : [];
...
export function isUnverifiedHistoricalLegalAnswer(answer: string): boolean {
  return answer.includes("収録している現行本文だけでは当時の内容を確定できないため、回答を保留します。");
}
...
export function verifiedCurrentSnapshotDate(
  ...
  const ageDays =
    (Date.parse(`${today}T00:00:00Z`) -
      Date.parse(`${checkedOn}T00:00:00Z`)) /
    86_400_000;
...
function legalPresentationIntent(query: string): LegalPresentationIntent | null {
  const normalized = compact(query);
  ...
  if (/例外/.test(normalized) && !rejectsExceptionPresentation) return "exception";
  ...
  if (/何条/.test(normalized) || (/条文/.test(normalized) && !rejectsExceptionPresentation)) {
    return "article";
  }
  return null;
}
...
function presentationTopicLabel(query: string): string {
  const normalized = compact(query);
  if (/フォー?クリフト/.test(normalized)) return "フォークリフト運転の資格・教育";
  if (/(?:足場|手すり|中さん|中桟)/.test(normalized)) return "足場の手すり等";
  if (/(?:酸欠|酸素欠乏|酸欠則)/.test(normalized)) {
    if (/(?:監視人|監視者|監視)/.test(normalized)) return "酸素欠乏危険作業の監視";
    if (/(?:測定|記録|保存)/.test(normalized)) return "酸素欠乏危険作業の測定・記録";
    if (/換気/.test(normalized)) return "酸素欠乏危険作業の換気";
    if (/(?:特別教育|教育)/.test(normalized)) return "酸素欠乏危険作業の特別教育";
    return "酸素欠乏危険作業";
  }
  if (/(?:有機溶剤|シンナー)/.test(normalized)) return "有機溶剤業務";
  if (/玉掛/.test(normalized)) return "玉掛け業務";
  if (/(?:電気作業|電気工事|充電電路)/.test(normalized)) return "電気作業の資格・教育";
  ...
  const indexes = articles.flatMap((article, index) => {
    ...
    if (/フォー?クリフト/.test(normalized)) {
      const relevant =
        ...
        (article.lawShort === "安衛法" && /^第?(?:59|61)条$/.test(article.articleNum));
    ...
    if (/玉掛/.test(normalized)) {
      const relevant =
        (article.lawShort === "安衛令" && /^第?(?:10|20)条$/.test(article.articleNum)) ||
        (article.lawShort === "クレーン則" && /^第?(?:221|222)条$/.test(article.articleNum));
      return relevant ? [index] : [];
    }
    if (/(?:電気作業|電気工事|充電電路)/.test(normalized)) {
      const directedWork =
      ...
      const relevant = directedWork
        ...
        : (article.lawShort === "電気工事士法" && /^第?[23]条$/.test(article.articleNum)) ||
...
function verifiedPresentationException(
  ...
  if (/フォー?クリフト/.test(normalized)) {
    ...
    const specialEducation = find(
      (article) =>
        article.lawShort === "安衛則" &&
        /^第?36条$/.test(article.articleNum) &&
        /一トン未満のフ[オォ]ー?クリフト|1トン未満のフォークリフト/.test(article.text),
    );
    const restrictedWork = find(
      (article) =>
        article.lawShort === "安衛令" &&
        /^第?20条$/.test(article.articleNum) &&
        /一トン以上のフ[オォ]ー?クリフト|1トン以上のフォークリフト/.test(article.text),
    );
  ...
  if (/(?:酸欠|酸素欠乏|酸欠則)/.test(normalized) && /換気/.test(normalized)) {
    const ventilation = find((article) =>
      article.lawShort === "酸欠則" &&
      /^第?5条$/.test(article.articleNum) &&
      /爆発、酸化等を防止するため換気することができない場合|作業の性質上換気することが著しく困難/.test(article.text),
    );
    if (ventilation >= 0) {
      const protection = find((article) =>
  ...
  if (/(?:有機溶剤|シンナー)/.test(normalized)) {
    const temporary = find((article) =>
      article.lawShort === "有機則" &&
      /^第?8条$/.test(article.articleNum) &&
      /臨時に有機溶剤業務/.test(article.text),
    );
    const shortTime = find((article) =>
      article.lawShort === "有機則" &&
...
function isRequestedVerifiedMaterial(article: LawArticle, material: string): boolean {
...
function presentationConclusion(
  ...
  if (intent === "article" && hasExplicitProvisionUnit && !/何条/.test(normalized)) {
    return null;
  }
    ...
    if (matching.length > 0) {
      return {
        conclusion: `${topic}について確認できた該当${intent === "paragraph" ? "項" : "号"}は、${matching
          .map(({ index, unit }) =>
            citedProvision(index, intent === "paragraph" ? unit.paragraph : unit.item),
          )
          .join("、")}です。`,
        conditions: complementary.length > 0
          ? [
              `ほかの取得根拠は${complementary
                .map(({ index, unit }) =>
                  citedProvision(index, intent === "paragraph" ? unit.item : unit.paragraph),
                )
                .join("、")}の単位で規定されており、一つの${intent === "paragraph" ? "項" : "号"}にはまとめられません。`,
            ]
          : [],
      };
    }
  ...
  const supportedException = verifiedPresentationException(query, articles, indexes);
...
function knownConclusion(
  ...
  if (highLiftIntent.fallProtection) {
    ...
    if (fallProtectionDuty >= 0) {
      return {
        conclusion:
          `高所作業車の作業床上では、事業者は労働者に要求性能墜落制止用器具等を使用させ、労働者本人も使用しなければなりません。直接根拠は安衛則194条の22です。${marker(fallProtectionDuty)}`,
        conditions: [
  ...
  if (electricWork && hasElectricQualificationSources && /公式原文/.test(normalized)) {
    return {
      conclusion:
        `公式原文は、根拠欄の電気工事士法2条・3条、安衛法59条3項、安衛則36条4号の各「公式原文」リンクから確認できます。${markers(...electricQualificationSources)}`,
      conditions: [
        `配線・設備工事は電気工事士法、充電電路の敷設・点検・修理・操作等は安衛法・安衛則の根拠を開いてください。${markers(...electricQualificationSources)}`,
      ],
    };
  }
  if (electricWork && hasElectricQualificationSources && /(?:何条|条文)/.test(normalized)) {
    return {
      conclusion:
        `主な根拠条文は、電気工事の範囲を定める電気工事士法2条、従事制限を定める同法3条、特別教育の根拠となる安衛法59条3項と安衛則36条4号です。${markers(...electricQualificationSources)}`,
      conditions: [
        `作業主任者ではなく作業指揮者を確認する場合は、対象作業を列挙する安衛則350条を別に確認します。`,
      ],
    };
  }
  if (electricWork && hasElectricQualificationSources && /何項/.test(normalized)) {
    return {
      conclusion:
        `特別教育の直接の根拠は安衛法59条3項です。電気工事士法は、2条3項が「電気工事」の定義、3条1項〜4項が設備・工事区分ごとの従事制限を定めています。${markers(...electricQualificationSources)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `充電電路の敷設・点検・修理・操作等に関する特別教育の対象業務は、安衛則36条4号です。電気工事士法3条は号ではなく、工事区分を各項で定めています。${markers(electricianRestriction, electricSpecialEducationWork)}`,
      conditions: [],
    };
  }
  if (electricWork && hasElectricQualificationSources && /告示/.test(normalized)) {
    return {
      conclusion:
        `この電気作業の資格・教育回答で確認済みの直接根拠は法律と省令で、関連告示は現在の検証済み回答根拠に含めていません。${markers(...electricQualificationSources)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `主な例外は制度ごとに異なります。安衛則36条4号の低圧業務では、対地電圧50V以下のものと、電信用・電話用等で感電危害のおそれがないものが同号の対象から除かれます。${marker(electricSpecialEducationWork)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `電気工事士法上の従事制限と、安衛法上の特別教育は別制度のため、開始日は一つではありません。現在の検証済み条文が基準日2026年8月3日に施行中であることは確認できますが、この回答の根拠metadataだけでは各制度の最初の施行日を確定できないため、日付は断定しません。${markers(...electricQualificationSources)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `電気作業すべてに一律の作業指揮者を置く規定ではありません。安衛則339条、341条1項、342条1項、344条1項または345条1項の作業では、作業の指揮者を定め、方法・順序の周知と直接指揮等を行わせます。${marker(electricWorkController)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `はい。低圧の充電電路の敷設または修理は、安衛法59条3項と安衛則36条4号に基づく特別教育が必要です。${markers(electricSpecialEducationDuty, electricSpecialEducationWork)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `安衛法59条1項は、労働者を雇い入れたとき、その業務に関する安全・衛生教育を行うよう事業者に求めています。${marker(employmentEducationDuty)} 安衛則35条は、当該労働者へ遅滞なく必要事項を教育するよう定めています。${marker(employmentEducationItems)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `「作業主任者」はすべての作業に共通する資格ではなく、政令で指定された作業について選任する制度です。作業名が分からない段階では要否を一つに確定できません。${markers(workLeaderDuty, workLeaderListedWork)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `特別教育は、危険・有害な業務として厚生労働省令で指定された作業に就かせるときに必要です。作業名が分からない段階では対象号を一つに確定できません。${markers(genericEducationDuty, genericSpecialEducationWork)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `必要な資格・教育は作業で変わります。現在の情報だけでは一つに確定できませんが、主な制度は、指定された危険・有害業務の教育、就業制限業務の免許・技能講習等、指定作業の作業主任者です。${markers(genericEducationDuty, genericRestrictedWork, workLeaderDuty)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `安衛法14条は、対象作業の区分に応じて作業主任者を選任し、労働者の指揮等を行わせるよう定めています。${marker(workLeaderDuty)} 安衛令6条は、「法第十四条の政令で定める作業」（指定された作業）を列挙しています。${marker(workLeaderListedWork)} 電気作業では、安衛則350条が安衛則339条・341条1項・342条1項・344条1項・345条1項の作業を行うときに作業の指揮者を定めるよう求めています。${marker(electricalWorkController)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `つり足場を除く足場の作業床は、幅40cm以上が必要です。${marker(scaffold)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `高さ2m以上の作業床の端や開口部で墜落のおそれがある箇所には、囲い・手すり・覆い等を設ける必要があります。${marker(openingProtection)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        measuredHeight !== null && Number.isFinite(measuredHeight) && measuredHeight < 85
  ...
  const forkliftTraining = articleIndex(
    ...
    (article) =>
      article.lawShort === "安衛則" &&
      /^第?36条$/.test(article.articleNum) &&
      /1トン未満のフォークリフト|一トン未満のフ[オォ]ークリフト/.test(article.text),
  );
  ...
  if (/フォー?クリフト/.test(normalized) && forkliftIntent.qualification) {
    if (
      ...
      return {
        conclusion:
          `最大荷重1トン以上のフォークリフト運転は、フォークリフト運転技能講習の修了者等に限られます。${markers(restrictedWorkDuty, forkliftDecree)}`,
        conditions: [
    ...
    if (
      ...
      return {
        conclusion:
          `最大荷重1トン未満のフォークリフト運転には、特別教育が必要です。${markers(safetyEducationDuty, forkliftTraining)}`,
        conditions: [
    ...
    if (
      ...
      return {
        conclusion:
          `安衛則36条5号は、最大荷重1トン未満のフォークリフト運転を掲げています。${marker(forkliftTraining)} 安衛法59条3項により、省令で定める危険・有害業務に就かせるときは特別教育が必要です。${marker(safetyEducationDuty)} 最大荷重1トン以上のフォークリフト運転は、安衛令20条11号の就業制限業務です。${marker(forkliftDecree)} 安衛法61条は、その業務を技能講習修了者等の所定資格を持つ者に限っています。${marker(restrictedWorkDuty)}`,
        conditions: [
  ...
  const slingLoadDefinition = articleIndex(
    ...
    (article) =>
      article.lawShort === "安衛令" &&
      /^第?10条$/.test(article.articleNum) &&
      /構造及び材料に応じて負荷させることができる最大の荷重/.test(
        article.text,
      ),
  );
  ...
  if (
    /玉掛/.test(normalized) &&
    (isDefinitelyAtLeast(statedLoad, 1) || /何(?:t|トン)から/.test(normalized)) &&
    (sling >= 0 || slingRule >= 0)
  ) {
    ...
    if (
      ...
      return {
        conclusion:
          `つり上げ荷重1トン以上のクレーン・移動式クレーン・デリックの玉掛けは、就業制限の対象です。${marker(sling)} その業務は、玉掛け技能講習の修了者等に限られます。${marker(slingRule)} つり上げ荷重1トン未満のクレーン等の玉掛けでは、事業者は特別教育を行わなければなりません。${marker(slingSpecialEducation)}`,
        conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `つり上げ荷重1トン未満のクレーン等の玉掛けには、特別教育が必要です。${marker(slingSpecialEducation)}`,
      conditions: [
        ...(slingLoadDefinition >= 0
          ? [
  ...
  if (/移動式クレーン/.test(normalized) && statedLoad !== null) {
    if (isDefinitelyBelow(statedLoad, 1) && mobileCraneSpecialEducation >= 0) {
      return {
        conclusion:
          `つり上げ荷重1トン未満の移動式クレーン運転には、特別教育が必要です。${marker(mobileCraneSpecialEducation)}`,
        conditions: [
    ...
    if (
      ...
      return {
        conclusion:
          `つり上げ荷重1トン以上5トン未満の移動式クレーン運転は、小型移動式クレーン運転技能講習の修了者が行えます。${marker(mobileCraneRestriction)}`,
        conditions: [
    ...
    if (isDefinitelyAtLeast(statedLoad, 5) && mobileCraneRestriction >= 0) {
      return {
        conclusion:
          `つり上げ荷重5トン以上の移動式クレーン運転には、移動式クレーン運転士免許が必要です。${marker(mobileCraneRestriction)}`,
        conditions: [
  ...
  if (
    ...
    if (organicHealthScopeIntent) {
      return {
        conclusion:
          `有機則29条1項の健康診断対象業務は、屋内作業場等で行う有機溶剤業務のうち、同則3条1項の場合の業務を除くものです。第三種有機溶剤等はタンク等の内部で行う業務に限られます。${marker(organicHealthExam)}`,
        conditions: [],
      };
    }
    return {
      conclusion:
        `有機則29条2項により、同条1項の対象業務に常時従事する労働者には、雇入れ時、配置替え時、その後6か月以内ごとに1回、医師による健康診断が必要です。${marker(organicHealthExam)}`,
      conditions: [],
    };
  }
  ...
  const organicIndoorEquipment = articleIndex(
    ...
    (article) =>
      article.lawShort === "有機則" &&
      /^第?5条$/.test(article.articleNum) &&
      /密閉する設備、局所排気装置又はプッシュプル型換気装置/.test(
        article.text,
      ),
  );
  ...
  const organicTankOutside = /(?:タンク等?の?外|タンク外|内部以外|それ以外の屋内)/.test(
    normalized,
  );
  ...
  if (
    ...
    if (organicTankOutside) {
      return {
        conclusion:
          `第三種有機溶剤等について、有機則6条の設備義務はタンク等の内部で行う業務が対象です。タンク等の内部以外の屋内という今回の条件には、同条の設備義務は適用されません。${marker(organicTankEquipment)}`,
        conditions: [
  ...
  if (
    /(?:酸欠|酸素欠乏)/.test(normalized) &&
    /(?:測定記録|記録事項|何を記録|記録.*(?:内容|保存)|保存.*何年|何年.*保存|第?3条第?2項)/.test(normalized) &&
    oxygenDeficiencyMeasurement >= 0
  ) {
    return {
      conclusion:
        `酸欠則3条2項は、測定のつど、①測定日時、②測定方法、③測定箇所、④測定条件、⑤測定結果、⑥測定を実施した者の氏名を記録し、3年間保存するよう定めています。${marker(oxygenDeficiencyMeasurement)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `はい。対象作業場では、その日の作業開始前に酸素濃度を測定します。第二種酸素欠乏危険作業では、酸素に加えて硫化水素の濃度も測定します。${marker(oxygenDeficiencyMeasurement)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `はい。対象となる作業場では、その日の作業開始前に空気中の酸素濃度を測定しなければなりません。第二種酸素欠乏危険作業では、硫化水素濃度も測定します。${marker(oxygenDeficiencyMeasurement)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `第二種酸素欠乏危険作業で換気により保つ硫化水素濃度は、100万分の10以下、すなわち10ppm以下です。${marker(oxygenDeficiencyVentilation)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `酸素欠乏危険作業で換気により保つ基準は、酸素濃度18%以上です。酸素濃度18%未満は「酸素欠乏」と定義されています。${markers(oxygenDeficiencyDefinition, oxygenDeficiencyVentilation)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `酸欠則12条1項の特別教育5科目は、①酸素欠乏の発生の原因、②酸素欠乏症の症状、③空気呼吸器等の使用方法、④事故時の退避・救急そ生方法、⑤その他の酸素欠乏症防止に必要な事項です。${marker(oxygenDeficiencyEducation)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `酸欠則12条1項は第一種の特別教育5科目を定め、2項がその規定を第二種にも準用します。その際、第1号の「酸素欠乏」を「酸素欠乏等」、第2号・第5号の「酸素欠乏症」を「酸素欠乏症等」に読み替えます。${marker(oxygenDeficiencyEducation)}`,
      conditions: [
  ...
  if (
    /(?:酸欠|酸素欠乏)/.test(normalized) &&
    /(?:換気.*(?:例外|できない|困難)|保護具|空気呼吸器|酸素呼吸器|送気マスク)/.test(normalized) &&
    oxygenDeficiencyVentilation >= 0 &&
    oxygenDeficiencyProtection >= 0
  ) {
    return {
      conclusion:
        `爆発・酸化防止のため換気できない場合、または作業の性質上換気が著しく困難な場合は換気義務の例外です。その場合でも、同時就業者数以上の空気呼吸器等を備え、労働者に使用させなければなりません。${markers(oxygenDeficiencyVentilation, oxygenDeficiencyProtection)}`,
      conditions: [],
    };
  }
  if (
    ...
    return {
      conclusion:
        `酸素欠乏危険作業では、原則として酸素濃度を18%以上に保つよう換気が必要です。第二種では、硫化水素濃度も100万分の10以下に保ちます。${marker(oxygenDeficiencyVentilation)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `酸素欠乏危険作業では、作業を常時監視し、異常時に作業主任者などへ直ちに通報する者を置く等の措置が必要です。${marker(oxygenDeficiencyMonitor)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `高さ2m以上でも、一律にフルハーネス型と決まるわけではありません。作業床を設けにくい場所や、作業床の端・開口部で囲い等を設けにくい場合は、要求性能墜落制止用器具を使用させる等の措置が必要です。${markers(fallArrestWorkFloor, fallArrestOpening)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `高さ2m以上で作業床を設けることが困難な場所において、フルハーネス型を用いる作業には特別教育が必要です。${markers(safetyEducationDuty, specialEducation)}`,
      conditions: [
  ...
  if (
    ...
    if (isUnderTwo && highLiftDefinition >= 0) {
      return {
        conclusion:
          `銘板・仕様上の作業床最高高さが2m未満なら、安衛令10条7号の「高所作業車」には該当せず、同区分の運転資格判定の対象外です。${marker(highLiftDefinition)}`,
        conditions: [
    ...
    if (isUnderTen && safetyEducationDuty >= 0) {
      return {
        conclusion:
          `銘板・仕様上の作業床最高高さが2m以上10m未満の高所作業車運転には、特別教育が必要です。${markers(safetyEducationDuty, specialEducation, highLiftDefinition)}`,
        conditions: [
    ...
    if (isTenOrMore && highLiftDecree >= 0 && restrictedWorkDuty >= 0) {
      return {
        conclusion:
          `銘板・仕様上の作業床最高高さが10m以上の高所作業車運転は、高所作業車運転技能講習の修了者等に限られます。${markers(restrictedWorkDuty, highLiftDecree, highLiftDefinition)}`,
        conditions: [
    ...
    if (
      ...
      if (highLiftDefinition < 0 && safetyEducationDuty >= 0) {
        return {
          conclusion:
            `作業床の高さ10m未満は特別教育が必要で、10m以上は高所作業車運転技能講習の修了者等に限られます。${markers(specialEducation, safetyEducationDuty, restrictedWorkDuty, highLiftDecree)}`,
          conditions: [
      ...
      return {
        conclusion:
          `作業床の高さが2m以上のものが、安衛令10条7号の「高所作業車」です。${marker(highLiftDefinition)} 作業床の高さが10m未満の高所作業車運転が、安衛則36条10号の5に掲げられています。${marker(specialEducation)} 作業床の高さが10m以上の高所作業車運転は、安衛令20条15号の就業制限業務です。${marker(highLiftDecree)} 安衛法61条は、政令で定める就業制限業務を技能講習修了者等の所定資格を持つ者に限っています。${marker(restrictedWorkDuty)}`,
        conditions: [
  ...
  if (
    ...
    const asbestosTargetCount = [buildingTarget, structureTarget, shipTarget].filter(Boolean).length;
    if (asbestosTargetCount === 1 && shipTarget) {
      return {
        conclusion:
          `船舶では、石綿則3条の事前調査対象は鋼製の船舶に限られます。その解体・改修前の事前調査は、船舶石綿含有資材調査者または同等以上の知識を有すると認められる者が行い、この調査者要件は令和5年10月1日から適用されています。${markers(asbestosSurvey, asbestosSurveyQualificationNotice)}`,
        conditions: [
    ...
    if (asbestosTargetCount === 1 && buildingTarget) {
      return {
        conclusion:
          `建築物の解体・改修前の事前調査は、原則として一般建築物石綿含有建材調査者、特定建築物石綿含有建材調査者または同等以上の能力を有すると認められる者が行い、この調査者要件は令和5年10月1日から適用されています。${markers(asbestosSurvey, asbestosSurveyQualificationNotice)}`,
        conditions: [
    ...
    if (asbestosTargetCount === 1 && structureTarget) {
      return {
        conclusion:
          `工作物の区分により、工作物石綿事前調査者、または一般・特定建築物石綿含有建材調査者等が事前調査を行います。この調査者要件は令和8年1月1日から適用されています。${markers(asbestosSurvey, asbestosSurveyQualificationNotice)}`,
        conditions: [
    ...
    return {
      conclusion:
        `石綿の事前調査を行える人は対象で変わります。建築物は建築物石綿含有建材調査者等、鋼製船舶は船舶石綿含有資材調査者等、所定の工作物は工作物石綿事前調査者等です。建築物・鋼製船舶の調査者要件は令和5年10月1日から、工作物は令和8年1月1日から適用されています。${markers(asbestosSurvey, asbestosSurveyQualificationNotice)}`,
      conditions: [
  ...
  if (
    ...
    return {
      conclusion:
        `熱中症のおそれがある作業では、症状の自覚や疑いを報告させる体制を整備し、作業従事者へ周知する必要があります。加えて、作業場ごとに作業からの離脱、身体の冷却、必要に応じた受診等の措置内容と実施手順をあらかじめ定め、周知しなければなりません。${marker(heat)}`,
      conditions: [
...
export function buildServiceFirstLegalAnswer(input: {
  ...
  const articles = input.articles
    ...
    .slice(0, 12);
  if (articles.length === 0) return buildServiceFirstNoHitAnswer(input.query, now);
  ...
  const requestedParagraphArticle = requestedParagraph
    ? { ...articles[0]!, text: requestedParagraph.text, itemNumberMap: undefined }
    : articles[0]!;
  ...
  const explicitlyRequestedItemSelection = explicitlyRequestedItemSelectionLabel(
    input.query,
    articles[0]!.articleNum,
  );
  const explicitlyRequestedItems = explicitlyRequestedItemLabelsForArticle.flatMap(
    (label) => {
  ...
  const explicitMultipleItemsConclusion =
    ...
    explicitlyRequestedItems.length === explicitlyRequestedItemLabelsForArticle.length
