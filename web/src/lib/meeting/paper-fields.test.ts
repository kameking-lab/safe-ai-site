...
describe("S1（第一弾・第二弾）: 打合せ用紙フィールドマップ", () => {
  it("13の静的欄が定義され、記入順が一筆書きになっている（循環なし・点検項目なしなら最後は次欄なし）", () => {
    expect(MEETING_PAPER_FIELD_ORDER).toHaveLength(13);
    // 点検項目カテゴリが有ると supervisorComment の次にチェーンが延びる（第六弾）ため、
    // 静的欄13個ぶんの一筆書きだけを検証するここでは checklist を空にする。
    const record = recordWithNoContractors({ checklist: [] });
...
describe("S1（第二弾）: 明日のイベント5欄＋統括安全責任者コメントの記入順チェーン", () => {
  it("各社マトリクス・点検項目無しなら作成担当者(author)の次は安全大会(safetyMeeting)、最終欄は統括安全責任者コメント(supervisorComment)で次欄なし", () => {
    const record = recordWithNoContractors({ checklist: [] });
...
describe("S1（第六弾）: 点検項目8カテゴリ（カテゴリ単位のキー）のフィールド定義・記入順チェーン", () => {
  it("checklistFieldKey / parseChecklistFieldKey が往復する。isMeetingPaperFieldKey は点検項目欄も認識", () => {
    const key = checklistFieldKey("general");
    expect(key).toBe("checklist.general");
    expect(parseChecklistFieldKey(key)).toEqual({ categoryKey: "general" });
    expect(parseChecklistFieldKey("bogus")).toBeNull();
    expect(parseChecklistFieldKey("siteName")).toBeNull();
    expect(isMeetingPaperFieldKey(key)).toBe(true);
  });

  it("getMeetingPaperFieldDef は公式8カテゴリそれぞれのラベルを返し、未知のカテゴリkeyは汎用ラベルにフォールバックする", () => {
    expect(getMeetingPaperFieldDef(checklistFieldKey("general"))).toMatchObject({
      label: "点検（一般事項）",
      type: "checklist",
      checklistCategoryKey: "general",
    });
    expect(getMeetingPaperFieldDef(checklistFieldKey("scaffold"))).toMatchObject({ label: "点検（足場）", type: "checklist" });
    expect(getMeetingPaperFieldDef(checklistFieldKey("unknown-cat"))).toMatchObject({ label: "点検（点検項目）", type: "checklist" });
  });

  it("点検項目は既定値(na=該当無)が正当な回答のため isEmpty は常にfalse（未記入ハイライト・zoom-to-cell対象外）", () => {
    const record = buildDefaultMeetingRecord();
    for (const cat of record.checklist) {
      expect(getMeetingPaperFieldDef(checklistFieldKey(cat.key)).isEmpty(record)).toBe(false);
    }
    expect(emptyMeetingPaperFieldKeys(record).has(checklistFieldKey("general"))).toBe(false);
  });

  it("setChecklistItemStatus は該当カテゴリ・該当項目のみをイミュータブルに更新する", () => {
    const record = buildDefaultMeetingRecord();
    const cat = record.checklist[0]!;
    const item = cat.items[0]!;
    const patch = setChecklistItemStatus(record, cat.key, item.key, "ng");
  ...
  it("記入順チェーン: 統括安全責任者コメント(supervisorComment)の次は1カテゴリ目→…→最終カテゴリの次は次欄なし", () => {
    const record = buildDefaultMeetingRecord();
    const expected: MeetingPaperFieldKey[] = ["supervisorComment", ...record.checklist.map((c) => checklistFieldKey(c.key))];
    let key: MeetingPaperFieldKey | undefined = "supervisorComment";
    const visited: MeetingPaperFieldKey[] = [];
    while (key) {
      expect(visited).not.toContain(key);
      visited.push(key);
  ...
  it("点検項目カテゴリが1件も無ければ supervisorComment の次は次欄なし（従来どおり）", () => {
    const record = recordWithNoContractors({ checklist: [] });
    expect(nextMeetingPaperFieldKey("supervisorComment", record)).toBeUndefined();
  });
});
