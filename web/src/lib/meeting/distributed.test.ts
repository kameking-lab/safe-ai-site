...
describe("要件1: 同時入力でデータ消失しない（行レベル分離）", () => {
  ...
  it("楽観ロック: 表記差(Z と +00:00)は同一時刻＝競合扱いしない（実DB検証で発覚した誤検知の修正）", () => {
    // DBは "+00:00"、サーバー生成は "Z"。同一瞬間なら競合ではない（正当な更新を弾かない）。
    expect(hasWriteConflict("2026-06-04T12:47:21.366+00:00", "2026-06-04T12:47:21.366Z")).toBe(false);
    expect(hasWriteConflict("2026-06-04T12:47:21.366Z", "2026-06-04T12:47:21.366+00:00")).toBe(false);
    // 実際に時刻が違えば競合
    expect(hasWriteConflict("2026-06-04T12:47:22.000+00:00", "2026-06-04T12:47:21.366Z")).toBe(true);
  });
});
