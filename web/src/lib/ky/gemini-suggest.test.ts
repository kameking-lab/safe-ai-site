...
describe("parseHazardSuggestions", () => {
  it("正常なJSONを解析し評価値・ラベルを補完する", () => {
    ...
    expect(r[0].basis).toContain("出典支持未確認");
    expect(r[0].retrievedExampleIds).toEqual([]);
    expect(r[0].sourceUrls).toEqual([]);
  });
  ...
  it("```json フェンス付きでも解析できる", () => {
    const text =
      ...
      JSON.stringify({
        hazards: [
          { hazard: "感電", reduction: "検電", likelihood: 2, severity: 3 },
        ],
      }) +
      "\n```";
    const r = parseHazardSuggestions(text, EXAMPLES);
  ...
  it("reduction 欠落時は空文字になる", () => {
    const r = parseHazardSuggestions(
      JSON.stringify({ hazards: [{ hazard: "X" }] }),
      EXAMPLES,
    );
  ...
  it("hazards が配列でなければ空配列", () => {
    expect(
      parseHazardSuggestions(JSON.stringify({ hazards: "no" }), EXAMPLES),
    ).toEqual([]);
  });
  ...
  it("過去事例と無関係な危険は grounded=false", () => {
    const r = parseHazardSuggestions(
      JSON.stringify({ hazards: [{ hazard: "宇宙線被曝" }] }),
      EXAMPLES,
    );
    expect(r[0].grounded).toBe(false);
  });
...
describe("buildHazardPrompt", () => {
  ...
  it("事例ゼロなら確認済み一次資料なしの注記を含める", () => {
    const { user } = buildHazardPrompt("作業", []);
    expect(user).toContain("確認済み一次資料はありません");
  });
});
...
describe("generateHazardsWithGemini (generate 注入でネット無しテスト)", () => {
  it("有効な応答で提案を返す", async () => {
    const generate = vi.fn(async () =>
      JSON.stringify({
        hazards: [
          { hazard: "墜落", reduction: "親綱", likelihood: 3, severity: 3 },
        ],
      }),
    );
  ...
  it("解析不能な応答は throw（→ ルートで擬似AIへフォールバック）", async () => {
    ...
    await expect(
      generateHazardsWithGemini("x", EXAMPLES, generate),
    ).rejects.toThrow();
  });
...
describe("isGeminiConfigured", () => {
  beforeEach(() => {
    ...
    delete process.env.GEMINI_EXTERNAL_AI_ENABLED;
  });
  afterEach(() => {
    delete process.env.GEMINI_API_KEY;
    delete process.env.GOOGLE_API_KEY;
  ...
  it("実キーがあれば true（GOOGLE_API_KEY でも可）", () => {
    ...
    process.env.GEMINI_EXTERNAL_AI_ENABLED = "true";
    expect(isGeminiConfigured()).toBe(true);
  });
  it("キーだけでは明示的な本番AI flagがないため false", () => {
    process.env.GOOGLE_API_KEY = "AIza-xxxx";
    expect(isGeminiConfigured()).toBe(false);
  });
  it("previewではキーが存在しても false", () => {
    process.env.VERCEL_ENV = "preview";
    process.env.GEMINI_API_KEY = "AIza-preview-must-not-run";
    process.env.GEMINI_EXTERNAL_AI_ENABLED = "true";
    expect(isGeminiConfigured()).toBe(false);
  });
});
