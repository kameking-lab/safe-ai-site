...
describe('CATEGORY_META', () => {
  it('全カテゴリにラベルと配色を持つ', () => {
    for (const key of ['law', 'plain', 'notice', 'chemical', 'education', 'accident', 'precedent', 'glossary', 'faq', 'sign'] as const) {
...
describe('buildSearchIndex — 用語集（glossary）の収載', () => {
  ...
  it('countByCategory の all は glossary を含む全カテゴリ合計に一致する', async () => {
    ...
    expect(c.all).toBe(
      c.law + c.plain + c.revision + c.notice + c.chemical + c.equipment + c.education + c.accident + c.precedent + c.glossary + c.faq + c.sign + c.article + c.feature,
    );
  });
});
...
describe('buildSearchIndex — 化学物質（chemical）の個別詳細への深リンク', () => {
  ...
  it('深リンク先 CAS はすべて濃度基準DBのキー集合に解決する（幽霊URL 0）', async () => {
    const index = await buildSearchIndex();
    const casKeys = (await import('@/lib/cross-search/chemical-detail-cas.json'))
      .default as string[];
    const canonical = new Set(casKeys);
    const linkedCas = index
      .filter(
        (i) => i.category === 'chemical' && i.url.startsWith('/chemical-database/'),
      )
      .map((i) => decodeURIComponent(i.url.replace(/^\/chemical-database\//, '')));
...
describe('buildSearchIndex — 法令条文（law）の収載', () => {
  it('curated 中核法令が law カテゴリで /law-search へ深リンクされる', async () => {
    ...
    const laws = index.filter((i) => i.category === 'law' && !i.id.startsWith('law-navi-beppyo-'));
    const beppyo = index.filter((i) => i.category === 'law' && i.id.startsWith('law-navi-beppyo-'));
    ...
    expect(beppyo.every((i) => i.url.startsWith('/law-navi/beppyo#'))).toBe(true);
  });
...
describe('buildSearchIndex — 法令ナビ分野ページ・別表の収載と着地', () => {
  it('分野ページを feature カテゴリで収載し /law-navi/topics/<id> へリンクする', async () => {
    ...
    expect(topics.every((i) => i.url.startsWith('/law-navi/topics/'))).toBe(true);
  });
  ...
  it('別表を law カテゴリで収載し「別表第3」＝「別表第三」で特定化学物質へ着地する', async () => {
    ...
    const hitArabic = arabic.find((i) => i.id === 'law-navi-beppyo-anei-rei-beppyo-3');
    const hitKanji = kanji.find((i) => i.id === 'law-navi-beppyo-anei-rei-beppyo-3');
    expect(hitArabic, '別表第3で意味インデックスに着地しない').toBeDefined();
    expect(hitKanji, '別表第三（漢数字）で意味インデックスに着地しない').toBeDefined();
  ...
  it('「有機溶剤 別表」（意味からの2語AND）で別表第6の2へ着地する', async () => {
    ...
    expect(results.some((i) => i.id === 'law-navi-beppyo-anei-rei-beppyo-6-2')).toBe(true);
  });
  ...
  it('「粉じん 別表」「じん肺 別表」「裾切値」で該当別表へ着地する（LN-D2 完了条件）', async () => {
    ...
    expect(funjin.some((i) => i.id === 'law-navi-beppyo-funjin-soku-beppyo-1')).toBe(true);
    const jinpai = searchItems(index, 'じん肺 別表', 'all', 10);
    expect(jinpai.some((i) => i.id === 'law-navi-beppyo-jinpai-soku-beppyo')).toBe(true);
    const susokiri = searchItems(index, '裾切値', 'all', 10);
    expect(susokiri.some((i) => i.id === 'law-navi-beppyo-anei-soku-beppyo-2')).toBe(true);
  });
  ...
  it('第2陣13分野: 代表俗称で分野ページが最上位に着地する', async () => {
    ...
    for (const [q, topicId] of TOP1) {
      ...
      expect(results[0]?.id, `「${q}」の1位が law-navi-topic-${topicId} でない`).toBe(
        `law-navi-topic-${topicId}`
      );
    }
  });
  ...
  it('第2陣13分野: 俗称・別名で分野ページが上位3位以内に着地する', async () => {
    ...
    for (const [q, topicId] of TOP3) {
      const results = searchItems(index, q, 'all', 10);
      const pos = results.findIndex((i) => i.id === `law-navi-topic-${topicId}`);
      expect(
        pos >= 0 && pos < 3,
...
describe('buildSearchIndex — 現場ことば版（plain）の収載', () => {
  ...
  it('「研削といし」は現場ことばタブ（category=plain）で安衛則第118条にヒットする', async () => {
    ...
    const plainHit = results.find((r) => r.title === '安衛則 第118条（現場ことば）');
    expect(plainHit, `結果: ${results.map((r) => r.title).join(' / ')}`).toBeTruthy();
  ...
  it('「囲い」は現場ことばタブ（category=plain）で安衛則第519条にヒットする', async () => {
    ...
    const plainHit = results.find((r) => r.title === '安衛則 第519条（現場ことば）');
    expect(plainHit, `結果: ${results.map((r) => r.title).join(' / ')}`).toBeTruthy();
  });

  it('広いクエリで非plainの枠が埋まっているときでも、余り枠が無ければ all 集約結果には出ない（既存ヒットを押し出さない設計の確認）', async () => {
    const index = await buildSearchIndex();
    // 「研削といし」は education/glossary/notice/law だけで上位10件が埋まる広いクエリ。
    const results = searchItems(index, '研削といし', 'all', 10);
    expect(results.length).toBe(10);
    expect(results.every((r) => r.category !== 'plain')).toBe(true);
  });

  it('余り枠があるクエリでは all 集約結果にも現場ことばヒットが出る（ID固有のためcategoryタブ不要な狭いクエリ）', async () => {
    const index = await buildSearchIndex();
    const results = searchItems(index, '安衛則 第118条 研削といし 試運転', 'all', 10);
    expect(results.some((r) => r.category === 'plain' && r.title === '安衛則 第118条（現場ことば）')).toBe(true);
  });
  ...
  it('「足場 作業床」は原文条文（law・T1）が引き続き上位3位以内に出る（plain 追加が既存回帰を崩さない）', async () => {
    ...
    expect(top.some((r) => r.category === 'law' && r.title === '安衛則 第563条')).toBe(true);
  });
