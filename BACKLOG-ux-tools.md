...
- [x] 【柱0バッチ9/9】その他ツール(/strategy/plan-generator・/subsidies+calculator・/bcp・/organization・/insurance・/chemical-database・/goods・/leaflet・/newsletter)=各ツールの判定/件数結論カードと文字ダイエット。第1弾＝判定/件数ツール5本→/subsidies・/subsidies/calculator・/strategy/plan-generator・/chemical-database・/goods（PR #527 main反映済）。第2弾＝/insurance・/bcp・/organization 状態系結論カード（PR #531 CI待ち）。第3弾＝/newsletter 登録前後の状態カード＝未登録(info/青)→登録完了(safe/緑)に色帯切替（2026-06-14 ux-tool/batch9-newsletter-status-card・無読テスト7/7 PASS）。/leaflet は印刷ラッパー専用（最上部状態カードの対象外＝水増し回避でスキップ）。
